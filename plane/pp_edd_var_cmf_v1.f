...
!
	...
	REAL(KIND=LDP) MIDF(ND),HU(ND),HL(ND),HS(ND),MID_DTAU(ND)
	REAL(KIND=LDP) W(ND),WPREV(ND),PSI(ND),PSIPREV(ND)
	REAL(KIND=LDP) EPS(ND)
	REAL(KIND=LDP) EPS_PREV(ND)
	REAL(KIND=LDP) JNU(ND),JNUM1(ND),HNUM1(ND)
	REAL(KIND=LDP) DBB
	LOGICAL DIF
!
! Local variables.
