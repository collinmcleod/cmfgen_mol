...
!
	IF(FIRST_TIME)THEN
	  ...
	  WRITE(47,'(4X,A,12X,A,5(9X,A),20X,A)')'I','FREQ','  XM',' ETA',' CHI','ESEC','   F','XM(I-2)-XM(I+2)' 
	  WRITE(47,'(53X,A)')'XM_SAV(I-2)-XM_SAV(I+2)' 
...
!
	DO I=2,ND-1
	  ...
	  DO WHILE(XM(I) .LT. 0.0D0 .AND. ICNT .LT. 3)
	    WRITE(50,'(I3,2F14.6,5ES14.4)')I,FREQ,0.01D0*C_KMS/FREQ,XM(I),
...
!
	IF(MINVAL(XM(1:ND)) .LE. 0.0D0)THEN
	  IF(VERBOSE)THEN
	    ...
	    CALL WRITE_VEC(TA,ND,'J Vec',47)
	    CALL WRITE_VEC(XM,ND,'R^2.J Vec',47)
	    CALL WRITE_VEC(XM_SAV,ND,'XM Vec',47)
	  ...
	  ELSE
	    DO I=1,ND
	     IF(XM(I) .LE. 0.0D0)THEN
	       ...
	       WRITE(47,'(T87,5ES13.4)')                                           XM_SAV(MAX(1,I-2):MIN(I+2,ND))
	     END IF
	    END DO
	  END IF
	END IF
!
	IF(VERBOSE)THEN
	  WRITE(49,'(ES16.8,3X,A)')FREQ,'Next'
	  WRITE(49,'(4X,A,12(X,A))')'I',
	1  '         XM','  RSQJ_PREV','  RSQH_PREV','  RSQH_OLDT','     XM_SAV',
	1  '         HS','         HT','   HS.RSQHP','   HT_RSQHT','  PSP.RSJHP',' DJDT.JOLDT'
	  DO I=1,ND
	    WRITE(49,'(I5,12ES12.4)')I,XM(I),RSQJNU_PREV(I),RSQHNU_PREV(I),RSQHNU_OLDT(I),
	1             XM_SAV(I),HS(I),HT(I),HS(I)*RSQHNU_PREV(I),HT(I)*RSQHNU_OLDT(I),PSIPREV(I)*RSQJNU_PREV(I),
	1             DJDt(I)*RSQJNU_OLDt(I)*R_RAT_FOR_J
	  END DO
	END IF
!
	...
	END DO
!
!	IF(RECORDED_ERROR)THEN
!	  XM=XM_SAV
!	  DO I=2,ND-1
!	   IF(XM(I) .LT. 0.0D0)XM(I)=ABS(XM(I))
!	  END DO
!	  CALL SIMPTH(TA,TB,TC,XM,ND,1)
!	  WRITE(50,*)FREQ,MINVAL(XM(1:ND))
!	END IF
!
