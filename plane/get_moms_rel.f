...
!
	  TMP_VEC(1)=GRSQ(1)*HNU_AT_OB; TMP_VEC(NS+1)=GRSQ(ND)*NNU_AT_IB
...
!
	    TMP_VEC(1)=GRSQ(1)*HNU_AT_OB; TMP_VEC(NS+1)=GRSQ(NS)*NNU_AT_IB
	    DO I=1,NS-1
	      T1=0.5D0*(GAM_REL_STORE(I)+GAM_REL_STORE(I+1))
...
!
	...
	END IF
!
	IF(FREQ .LT. 11.44807 .AND. FREQ .GT. 11.44805 .AND. ND .EQ. ND_STORE)THEN
	  WRITE(200,*)FREQ
	  DO I=1,ND
