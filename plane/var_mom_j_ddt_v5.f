...
!
	...
	REAL(10), ALLOCATABLE :: DERIV_SCL_FAC(:)
	REAL(10), ALLOCATABLE :: COH_VEC(:)
!
	REAL(10) CHI_AT_INB_PREV
	REAL(10) DBB_PREV
	REAL(10) dDBBDT_PREV
!
...
! JNU and r^2 HNU.
!
	SUBROUTINE VAR_MOM_J_DDT_V5(ETA,CHI,ESEC,THETA,V,R,
...
!
	IF(XM(I) .LT. 0.0D0)THEN
	  ...
	  ELSE
	    I=ERROR_LU()
	    WRITE(I,*)'Unrecognized J_CHK_OPTION in VAR_MOM_J_DDT_V5: ',TRIM(J_CHK_OPTION)
	    STOP
	  END IF	
	END IF
!
...
!
	...
	ELSE
	  I=ERROR_LU()
	  WRITE(I,*)'Unrecognized H_CHK_OPTION in VAR_MOM_J_DDT_V5: ',TRIM(H_CHK_OPTION)
	  STOP
	END IF
!
! 
...
!
	CALL TUNE(1,'MOM_EDD')
	CALL EDD_J_HUB_VAR_V3(KI,RHS_dHdCHI,WORKMAT,
