...
!
	SUBROUTINE JGREY_HUB_DDT_V5(RJ,RSQ_HFLUX,CHI,CHI_PLANCK,R,CLUMP_FAC,
	1              VEL,SIGMA,POPS,
	...
	IMPLICIT NONE
!
! Altered 21-Mar-2018: Clumping taken into account for WORK and rad. decay terms.
!                        CHI and CHI_PLANCK should already be scaled by the clumping factor.
...
!
	...
	REAL(KIND=LDP) CLUMP_FAC(ND)		!Clumping factor
...
!
	...
	REAL(KIND=LDP) TIME_SEQ_NO              !Does not need to be an integer
...
!
	...
	E_RAD_DECAY=T1*E_RAD_DECAY*CLUMP_FAC
!
...
!
	...
	DO I=1,ND
	  ...
	  T3=( RJ(I)+(E_RAD_DECAY(I)-WORK(I))/MAX(0.1_LDP*CHI(I),CHI_PLANCK(I)) )
	  IF(T3 .GT. 0.0_LDP)THEN
	    T3=T1*(T3**0.25_LDP)
	  ELSE
	    T3=0.0D0
	  END IF
