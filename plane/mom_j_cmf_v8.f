...
!
	  DEALLOCATE ( RSQJNU )
	  DEALLOCATE ( RSQHNU )
	  DEALLOCATE ( K_ON_J_SAVE )
	  DEALLOCATE ( NMID_ON_HMID_SAVE )
	  DEALLOCATE ( NMID_ON_J_SAVE )
!
	  DEALLOCATE ( RSQJNU_PREV )
	  DEALLOCATE ( RSQHNU_PREV )
	  DEALLOCATE ( K_ON_J_PREV )
	  DEALLOCATE ( NMID_ON_HMID_PREV )
	  DEALLOCATE ( NMID_ON_J_PREV )
!
	  DEALLOCATE ( CON_GAM )
	  DEALLOCATE ( CON_GAMH )
	  DEALLOCATE ( AV_SIGMA )
...
!
	  ...
	  IF(IOS .NE. 0)THEN
	     WRITE(2,*)'Unable to allocate COEF memory in MOM_J_CMF_V8'
	     STOP
	  END IF
!
	  ALLOCATE ( V(ND) )
...
!
	  ...
	  ALLOCATE ( K_ON_J_SAVE(ND) )      	; K_ON_J_SAVE(1:ND)=0.0D0
	  ALLOCATE ( NMID_ON_HMID_SAVE(ND) )    ; NMID_ON_HMID_SAVE(1:ND)=0.0D0
	  ALLOCATE ( NMID_ON_J_SAVE(ND) )   	; NMID_ON_J_SAVE(1:ND)=0.0D0
!
...
!
	IF(MINVAL(XM(1:ND)) .LE. 0.0D0)THEN
	   ...
	   CALL WRITV(K_ON_J,ND,'K_ON_J (FEDD) Vec',47)
	   CALL WRITV(NMID_ON_HMID,ND,'NMID_ON_HMID (G) Vec',47)
