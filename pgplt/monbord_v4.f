...
C
	SUBROUTINE MONBORD_V4(XPAR,XINC,XNUMST,IXTICK,IDX,
...
C Right edge
C
	IF(YLAB_FILE .NE. ' ' .AND. NORMAL_R_AXIS)THEN
	  OPEN(UNIT=10,FILE=YLAB_FILE,STATUS='OLD',ACTION='READ',IOSTAT=IOS)
	  IF(IOS .EQ. 0)THEN
	    TAKE_LOG=.FALSE.
	    READ(10,'(A)')STRING
	    READ(STRING,*)NY
	    IF(IOS .NE. 0)THEN
	       WRITE(6,*)'Error readn NY form YLAB_FILE: ',TRIM(YLAB_FILE)
	       YLAB_FILE=' '
	    END IF
	    IF(INDEX(STRING,'LOG') .NE. 0 .OR. INDEX(STRING,'log') .NE. 0)TAKE_LOG=.TRUE.
	    DO I=1,NY
	      READ(10,*)Y
	      IF(IOS .NE. 0)THEN
	        WRITE(6,*)'Error reading values from YLAB_FILE: ',TRIM(YLAB_FILE),' Record=',I+1
	        YLAB_FILE=' '
	        EXIT
	      END IF
	      IF(TAKE_LOG)Y=LOG10(Y)
	    ...
	    CLOSE(UNIT=10)
	  ELSE
	    WRITE(6,*)'Unable to open file with Y ordinate labels'
	  END IF
	ELSE IF( NORMAL_R_AXIS .AND.
...
C
	I=LEN(TOP_AX_OPT)
	IF(I .GT. 6)I=6
	IF(XLAB_FILE .NE. ' ')THEN
	  OPEN(UNIT=10,FILE=XLAB_FILE,STATUS='OLD',ACTION='READ',IOSTAT=IOS)
	  IF(IOS .EQ. 0)THEN
	    TAKE_LOG=.FALSE.
	    READ(10,'(A)')STRING
	    READ(STRING,*,IOSTAT=IOS)NX
	    IF(IOS .NE. 0)THEN
	       WRITE(6,*)'Error reading NX from XLAB_FILE: ',TRIM(XLAB_FILE)
	       XLAB_FILE=' '
	       NX=0
	    END IF
	    IF(INDEX(STRING,'LOG') .NE. 0 .OR. INDEX(STRING,'log') .NE. 0)TAKE_LOG=.TRUE.
	    DO I=1,NX
	      ...
	      IF(IOS .NE. 0)THEN
	         WRITE(6,*)'Error readng values from XLAB_FILE: ',TRIM(XLAB_FILE),' Record=',I+1
	         XLAB_FILE=' '
	         EXIT
	      END IF
	      IF(TAKE_LOG)X=LOG10(X)
	      CALL PGMOVE(X,YPAR(2))
	      CALL PGDRAW(X,YPAR(2)-FNTICK(YTCK_SIZE,IXTICK,I-IXST))
	    END DO
	    CLOSE(UNIT=10)
	  ELSE
	    WRITE(6,*)'Unable to open file with X abscissa labels'
	  END IF
	ELSE IF(AX_OPT .EQ. 'LOGX' .OR. AX_OPT .EQ. 'LOGXY')THEN
	  DO I=0,NX
	    DO J=1,9
	...
	1         (XPAR(1) .GE. X .AND. XPAR(2) .LE. X) )THEN
	      IF(MOD(I,2) .NE. 0 .OR. INDEX(TOP_AX_OPT,'/ALL') .NE. 0)THEN
	        Y=SCED(I)
	        CALL PGMOVE(X,YPAR(2))
...
C
	IF(YLAB_FILE .NE. ' ')THEN
	  ...
	  ELSE
	    WRITE(6,*)'Unable to open file with Y ordinate labels'
	  END IF 
	ELSE IF(AX_OPT .EQ. 'LOGY' .OR. AX_OPT .EQ. 'LOGXY')THEN
	  DO I=0,NY
	    DO J=1,9
	      Y=YNUMST+(I-1)*YINC+LOG_OFFSET(J)
	      IF( MONINSIDE(Y,YPAR) )THEN
...
C
	IF(XLAB_FILE .NE. ' ')THEN
	  ...
	  IF(IOS .EQ. 0)THEN
	    ...
	    CLOSE(UNIT=10)
	  ELSE
	    WRITE(6,*)'Unable to open string with X labels'
	  END IF
!
	ELSE IF(AX_OPT .EQ. 'LOGX' .OR. AX_OPT .EQ. 'LOGXY')THEN
	  DO I=0,NX
