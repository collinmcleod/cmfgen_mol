...
!
	SUBROUTINE DRAW_EW_IDS(XPAR,YPAR,EXPCHAR,T_OUT)
	USE LINE_ID_MOD
...
!
	WRITE(6,*)'Entered DRAW_EW_IDS'
	IF(N_EW_IDS .NE. 0)THEN
	  CALL PGSCI(IONE)
	  ID_LOC=4
	  ID_ORIENT=90.0D0
	  ID_LOC_PG=1.0D0
	  IF(ID_VEC_BEG .GT. ID_VEC_END)ID_LOC=6
!
	  DO I=1,N_EW_IDS
	    WR_ID(I)=.TRUE.
	    DO J=1,N_OMIT_ID
...
!
	  DO I=1,N_EW_IDS
	    ...
	    IF(WR_ID(I) .AND. T1 .GT. 0.0D0)THEN
	      TMP_STR=' '
	      T1=ID_VEC_BEG+0.1*ABS(ID_VEC_BEG-ID_VEC_END)
	      IF(ID_VEC_BEG .GT. ID_VEC_END)T1=ID_VEC_END-0.1*ABS(ID_VEC_BEG-ID_VEC_END)
	...
	1                  XSTRPOS,YSTRPOS,TMP_STR,IONE)
	      ...
	      IF(T1 .GT. 0 .AND. T2 .GT. 0)THEN
	        ...
	        IF(ID_VEC_BEG .GT. ID_VEC_END)THEN
	          CALL PGMOVE(ID_WAVE(I),ID_VEC_BEG)
	          CALL PGDRAW(ID_WAVE_OFF(I),ID_VEC_END)
	        ELSE
