...
C
	...
	CHARACTER*(*) TITLE(N_TITLE)
	CHARACTER*(*) AX_OPT,TOP_AX_OPT
C
C Local paremeters
C
...
C
	...
	INTEGER*4 CI_SAV
C
	CHARACTER STR*4,XNUM*12,YNUM*12
	INTEGER*4 XNUM_LEN,YNUM_LEN
	REAL*4 LOG_OFFSET(9)
C
	CHARACTER*30 UC
	EXTERNAL UC
...
!
	INTEGER*4 PEN_TIT(N_TITLE)
	REAL*4 LEN_TIT(N_TITLE)
	REAL*4 HT_TIT(N_TITLE)
	CHARACTER*80 LOC_TIT(N_TITLE)
	INTEGER*4 PEN_XLAB
	INTEGER*4 PEN_YLAB
!
	CHARACTER*80 LOC_XLAB,LOC_YLAB
	LOGICAL MONINSIDE
C
