...
!
	...
	REAL(10) LINE_HEIGHT(NL_MAX)
	REAL(10) LINE_CENTER(NL_MAX)
	REAL(10) LINE_SIGMA(NL_MAX)
!
	REAL(10) TOLERANCE
...
! 
	  ...
	  END IF
!
! Note: These four arrays are also allocate/used in do_gaus_fit.f
!
...
!
	  IF(ALLOCATED(EW_TABLE))DEALLOCATE(EW_TABLE)
	  IF(ALLOCATED(LAM_TABLE))DEALLOCATE(LAM_TABLE)
