...
!
	...
	IF(IOS .NE. 0)THEN
	  ...
	  WRITE(LU_ER,*)'IOS=',IOS
	  IF(ASSOCIATED(CD(J)%XVEC))DEALLOCATE(CD(J)%XVEC)
	  IF(ASSOCIATED(CD(J)%DATA))DEALLOCATE(CD(J)%DATA)
	END IF
!
...
!
	...
	IF(IOS .NE. 0)THEN
	  ...
	  IF(ASSOCIATED(CD(J)%XVEC))DEALLOCATE(CD(J)%XVEC)
	  IF(ASSOCIATED(CD(J)%DATA))DEALLOCATE(CD(J)%DATA)
	  IF(ASSOCIATED(CD(J)%DATA))DEALLOCATE(CD(J)%EMAX)
	  IF(ASSOCIATED(CD(J)%DATA))DEALLOCATE(CD(J)%EMIN)
	END IF
!
	CD(J)%XVEC(1:NUM)=X(1:NUM)
!
