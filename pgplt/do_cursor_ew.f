...
!
	NPIX=1
	CALL GEN_IN(NPIX,'Number of pixels to average continuum location (must be odd)')
...
!
	  WRITE(6,'(A)')' '
	  WRITE(6,'(A)')' Use cursor to define line:'
	  WRITE(6,'(A)')'   Y - use cursor value for continuum location '
	  WRITE(6,'(A)')'   S - reset and start line selection again'
	  WRITE(6,'(A)')'     - Y value is average over data'
	  WRITE(6,'(A)')' '
!
	ELSE
...
!
	  ...
	  DO I=IST,IEND
	    ...
	    dX=(CD(IP)%XVEC(I+1)-CD(IP)%XVEC(I-1))/2
...
!
	  ...
	  IF(ABS(EW) .LT. 1.0D-10*YCONT)THEN
	    WRITE(6,*)'Possibe error EW is close to zero', EW/YCONT
	    WRITE(6,*)'Skipping this line. XST,END=',XST,XEND
	    GOTO 1000
	  END IF
	  XMEAN=XMEAN/EW
	  YINT=EW
	  YMEAN=EW/(CD(IP)%XVEC(IEND+1)-CD(IP)%XVEC(IST-1))
	  EW=EW/YCONT
	  EWL=EWL/(YCONT*0.99)
	  EWH=EWH/(YCONT*1.01)
!
