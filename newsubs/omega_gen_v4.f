...
!
	...
	IMPLICIT NONE
!
! Created 24-Oct-2023 : Changed to v4 to deal with molecules. New variable added in call
!
...
!
	...
	LOGICAL IS_MOL                  !indicates whether current species is a molecule
!
	INTEGER ERROR_LU
	LOGICAL SAME_N
	EXTERNAL ERROR_LU
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC) PRIVATE(I,J,FL,X,IFAIL,EX_E1X,GBAR,dln_GBAR_dlnT,G1,G2)
	DO I=1,NLEV
	  DO J=I+1,NLEV
	    IF(OMEGA(I,J) .EQ. 0.0_LDP .AND. EIN_A(I,J) .NE. 0.0_LDP .AND. .NOT. IS_MOL)THEN
	      FL=EDGE(I)-EDGE(J)
	      X=HDKT*FL/TEMP
	      IFAIL=0
