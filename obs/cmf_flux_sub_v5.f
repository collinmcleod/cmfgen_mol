...
!
	INTEGER*4, PARAMETER :: MAX_SIM=700
!
...
C
	...
	LOGICAL ALL_FREQ
        REAL*8 ACC_FREQ_END
	INTEGER*4 NPINS			!Points inserted for error calc (CMF).
	INTEGER*4 N_INS_OBS		!Points inserted for error calc (Observers Frame).
...
!
	IF(FL .LT. NU_FORCE .AND. LST_ITERATION .AND. WRITE_CMF_FORCE)THEN
	  ...
	  IF(.NOT. TMP_LOG)THEN
	    ...
	    CALL WRITE_DIRECT_INFO_V3(ND,I,DA_FILE_DATE,'CMF_FORCE_DATA',J)
	    OPEN(UNIT=82,FILE='CMF_FORCE_DATA',FORM='UNFORMATTED',
...
! 
	IF(WRITE_ETA_AND_CHI)THEN
	  ACCESS_F=5
	  I=WORD_SIZE*(ND+1)/UNIT_SIZE; J=82
	  CALL WRITE_DIRECT_INFO_V3(ND,I,DA_FILE_DATE,'CHI_DATA',J)
	  CALL WRITE_DIRECT_INFO_V3(ND,I,DA_FILE_DATE,'ETA_DATA',J)
	...
	1       ACCESS='DIRECT',STATUS='NEW',RECL=I,IOSTAT=IOS)
	  ...
	  DO ML=1,NCF
	    ...
	    WRITE(83,REC=ACCESS_F-1+ML)(CHI_CMF_ST(I,ML),I=1,ND),NU(ML)
	  END DO
	  CLOSE(UNIT=82)
	  CLOSE(UNIT=83)
	END IF
...
!
	IF(FL .LT. NU_FORCE .AND. WRITE_SOB_FORCE)THEN
	  ...
	  IF(.NOT. TMP_LOG)THEN
	    I=WORD_SIZE*(ND+1)/UNIT_SIZE; J=82
	    CALL WRITE_DIRECT_INFO_V3(ND,I,DA_FILE_DATE,'SOB_FORCE_DATA',J)
	    OPEN(UNIT=82,FILE='SOB_FORCE_DATA',FORM='UNFORMATTED',
	1       ACCESS='DIRECT',STATUS='NEW',RECL=I,IOSTAT=IOS)
	    WRITE(82,REC=EDD_CONT_REC)K,N_FORCE,ND
	  END IF
