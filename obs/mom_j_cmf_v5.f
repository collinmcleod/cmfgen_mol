...
C
	INTEGER*4 N_ERR_MAX,MOM_ERR_CNT
...
C
	...
	END DO
C
	IF(FREQ .LE.  2.531888D0 .AND. FREQ .GE.  2.529833D0)THEN
	  WRITE(133,*)' '
	  WRITE(133,*)'FREQ=',FREQ
	  CALL WRITV(XM,ND,'XM',133)
	  CALL WRITV(RSQHNU,ND-1,'RSQHNU',133)
	  CALL WRITV(HU,ND-1,'HU',133)
	  CALL WRITV(HL,ND-1,'HL',133)
	  CALL WRITV(HS,ND-1,'HS',133)
	  CALL WRITV(ESEC,ND,'ESEC',133)
	  CALL WRITV(ETA,ND,'ETA',133)
	  CALL WRITV(CHI,ND,'CHI',133)
	  CALL WRITV(Q,ND,'Q',133)
	  CALL WRITV(F,ND,'F',133)
	  CALL WRITV(RSQN_ON_RSQJ,ND,'RSQN_ON_RSQJ',133)
	  CALL WRITV(W,ND,'W',133)
	  CALL WRITV(WPREV,ND,'WPREV',133)
	  CALL WRITV(PSI,ND,'PSI',133)
	  CALL WRITV(PSIPREV,ND,'PSIPREV',133)
	  CALL WRITV(EPS,ND,'EPS',133)
	  CALL WRITV(EPS_PREV,ND,'EPS',133)
	END IF
!
