...
C
	...
	REAL(10) ETA(ND),CHI(ND),ESEC(ND),JCONT(ND),CHIL(ND),ETAL(ND)
	REAL(10) V(ND),SIGMA(ND),R(ND),P(NP)
C
	REAL(10) JQW(ND,NP),HQW(ND,NP),KQW(ND,NP),NQW(ND,NP)
	REAL(10) JNU(ND,NLF+1),HNU(ND,NLF+1),KNU(ND,NLF+1),NNU(ND,NLF+1)
	REAL(10) IN_HBC(NLF+1),HBC(NLF+1),NBC(NLF+1)
C
	REAL(10) PF(NLF),PROF(NLF),LFQW(NLF),WERFC(NLF)
	REAL(10) DBB,IC,FL,EW,CONT_INT
...
C
	...
	REAL(10) TCHI(NV),XM(NV),SOURCE(NV)
	REAL(10) UA(NV),UB(NV),UC(NV),VB(NV),VC(NV)
	REAL(10) GB(NV),H(NV),GAM(NV),GAMH(NV),Q(NV),QH(NV)
	REAL(10) AVM1(NV),CVM1(NV),JPREV(NV)
	REAL(10) AVCONT(NV),CVCONT(NV),dCHIdR(NV)
C
C Local variables.
C
	INTEGER I,J,LS,ML,NI,NIEXT
	REAL(10) OLDCHI,T1,DBC,DNU,TOR,MID_PRO,SCALE,HBLANK
	REAL(10) WERF_EXP,IBOUND,IMIN
C
