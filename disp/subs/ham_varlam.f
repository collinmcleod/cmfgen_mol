...
C
	...
	INTEGER LS,NC,NI
	REAL(10) TERF,IBOUND,DBC
	REAL(10) KI(NI),VSRCE(NI),RKB(NI),LINE_PRO(NI)
	REAL(10) DTAU(NI),R(NI),Z(NI)
	REAL(10) GB(NI),Q(NI),QH(NI),UK(NI),UKM1(NI)
	REAL(10) VKM1(NI),TCHI(NI),SOURCE(NI),CHI(NI),CHIL(NI),ETAL(NI)
C
C Local variables
C
	INTEGER NV,I,J,K
	PARAMETER (NV=100)
	REAL(10) T1,dTAdCHI,dTBdCHI,dTCdCHI,dVBdCHI,dVCdCHI
	REAL(10) WA(NV),WB(NV),dGBdTAU(NV),dGBdCHI(NV),dHdCHI(NV)
C
	IF(NI .GT. NV)THEN
