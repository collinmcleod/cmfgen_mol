...
C
	...
	REAL(10) JQW(ND,NP),HQW(ND,NP)
	REAL(10) JBAR(ND),ZNET(ND)
	REAL(10) JCONT(ND),JBLANK(ND),HBLANK(ND),JINT(ND)
	REAL(10) LAMLINE(ND),dZdCHIL(ND),dZdETAL(ND)
	REAL(10) PF(NLF),PROF(NLF),LFQW(NLF),WERFC(NLF)
	REAL(10) DBB,IC,AMASS,FL,EW,CONT_INT
...
C
	REAL(10) PROGDESC
	REAL(10) TA(NV),TB(NV),TC(NV),AV(NV),CV(NV),DTAU(NV),Z(NV)
	REAL(10) UA(NV),UB(NV),UC(NV)
	REAL(10) TCHI(NV),XM(NV),SOURCE(NV),VB(NV),VC(NV)
	REAL(10) GB(NV),H(NV),GAM(NV),GAMH(NV),Q(NV),QH(NV)
	REAL(10) JEX_SCAT(NV),BAR_WTS(NV),DEPTH_PRO(NV)
	REAL(10) AVM1(NV),CVM1(NV),dJBARdKI(NV),DIAG(NV)
	REAL(10) dJINTdS(NV),HCONT(NV),JINT_PREV(NV),HINT(NV)
	REAL(10) AVCONT(NV),CVCONT(NV),dCHIdR
	REAL(10) VSRCE(NV),dUdS(NV),dVdS_UP(NV),dVdS_LOW(NV)
	REAL(10) VKI(NV),RKB(NV),dUdKI(NV),dVdKI_UP(NV),dVdKI_LOW(NV)
C
C Local variables.
C
	INTEGER I,LS,ML,NI,NIEXT
	REAL(10) OLDCHI,T1,T2,DBC,VDBC,DNU,TOR,IBOUND,MID_PRO,SCALE
	REAL(10) WERF_EXP
C
...
C
	...
	1            *SQRT(R(ND)*R(ND)-P(LS)*P(LS))/R(ND)
	      END IF
	      OLDCHI=T1
	    END IF
...
C
	    IF(LS .LE. NC)THEN
	      DBC=DBB*DNU*SQRT(R(ND)*R(ND)-P(LS)*P(LS))
	1       /R(ND)/CHI(ND)
	    END IF
C
