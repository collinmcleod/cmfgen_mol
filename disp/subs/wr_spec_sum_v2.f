...
!
	...
	CHARACTER(LEN=120) HEADER_A
	CHARACTER(LEN=120) HEADER_B
!
...
!
	  ...
	  IF(ABS(XV(DPTH_INDX)) .GT. 0.1 .AND. ABS(XV(DPTH_INDX))  .LT. 100.0D0)THEN
	    WRITE(HEADER_B(J:),'(7X,A,I3,A,F8.3)')'X(',DPTH_INDX,')=',XV(DPTH_INDX)
	  ELSE
	    WRITE(HEADER_B(J:),'(7X,A,I3,A,ES8.3)')'X(',DPTH_INDX,')=',XV(DPTH_INDX)
	  END IF
