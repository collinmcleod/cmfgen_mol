	SUBROUTINE RD_ION_LOCATIONS(ION_ID,NION,ION_INDEX,NUM_IONS,NMAX,LU_IN)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
	INTEGER NMAX
...
!
	ION_ID=' '
!
	OPEN(UNIT=LU_IN,FILE='MODEL',STATUS='OLD',ACTION='READ')
	STRING=' '
...
!
	...
	DO WHILE(1 .EQ. 1)
	  IF(STRING .NE. ' ')THEN
	    ...
	    READ(STRING(K:),*)I,NION(NUM_IONS),T1,T1,T1,T1,I,ION_INDEX(NUM_IONS)
	  END IF 	
	  READ(LU_IN,'(A)')STRING
