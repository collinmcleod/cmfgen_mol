...
!
	...
	DO I=1,NMOD
	  ...
	  IF(NORM)THEN
	    ...
	    MAXIMUM=0.0D0; MINIMUM=10000.0D0
	    DO J=1,NCF
	      IF(XV(J) .GT. 6200 .AND. XV(J) .LT. 7000)THEN
	        IF(YV(J) .GT. MAXIMUM)THEN
	          MAXIMUM=YV(J)
	          IMAX=J
	        ELSE IF(YV(J) .LT. MINIMUM)THEN
	          MINIMUM=YV(J)
	          IMIN=J
	        END IF
	      END IF
	    ...
	    WRITE(20,*)SN_AGE(I),
	1         C_Mms*(XV(IMIN)/HA_WAVE-1.0D0),
	1         C_MmS*(XV(IMAX)/HA_WAVE-1.0D0)
	  ELSE
