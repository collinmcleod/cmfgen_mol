...
! Altered 18-Apr-2022 by Collin McLeod: added MOL_CR and MOL_DR variables
! Altered 11-Oct-2021 by Collin McLeod: added information and slight changes to treatments for molecules
!
...
!
! Above set to true for debugging (collin McLeod)
...
!
	...
	IF(ID .NE. NUM_SPECIES)THEN
	  ...
	  WRITE(LUER,*) 'Max number of species: ',NUM_SPECIES
	  WRITE(LUER,*) 'Number of species included: ',ID
	  STOP
	END IF
!
...
!
	...
	NT=EQNE+1
!
! NT will not be correct once I change how molecules are treated--currently it is counting
! one more equation than is necessary for each molecule.
!
C	NT = NT-5 !number of conservation equations which should not be included
!
...
!
	IF(EQNE .NE. I)THEN
	  ...
	  WRITE(LUER,*)'EQNE=',EQNE,' Should be ',I
	  STOP
	END IF
	IF(NT .NE. I+1)THEN
...
!
	  ...
	  IF(IOS .EQ. 0)ALLOCATE (ATM(ID)%MOL_CRXzV(ND),STAT=IOS)
	  IF(IOS .EQ. 0)ALLOCATE (ATM(ID)%MOL_DRXzV(ND),STAT=IOS)
