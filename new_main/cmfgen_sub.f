...
!
	...
	1             BA_CHK_FAC,NION,NT,NUM_BNDS,ND,DST,DEND)
	      T1=VCHI_ALL(NT,DPTH_INDX)*RJ(DPTH_INDX)
	      T2=(CHI(DPTH_INDX)-CHI_SCAT(DPTH_INDX))*VJ(NT,DIAG_INDX,DPTH_INDX)
	      T3=T1-VETA_ALL(NT,DPTH_INDX)+T2
	      IF(VERBOSE_OUTPUT)THEN
	        WRITE(200,'(I10,12ES18.8)')ML,FL,RJ(DPTH_INDX),T1,VETA_ALL(NT,DPTH_INDX),T2,T3,FQW(ML)*T3
	      END IF
	    END IF
!
	  END DO		!K=1,ND : DST,DEND
...
!
	...
	1                             BA_T(NT-3,DIAG_INDX,DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX)
	  ELSE
	    WRITE(199,'(I10,12ES18.8)')ML,FL,STEQ_T(DPTH_INDX),T1,FQW(ML)*ETA_NOSCAT(DPTH_INDX),
	1                             STEQ_T_SCL(DPTH_INDX),STEQ_T_NO_SCL(DPTH_INDX),
	1                             BA_T(NT-3,DIAG_INDX,DPTH_INDX)+BA_T_PAR(NT-3,DPTH_INDX),
...
10000	CONTINUE
	IF(VERBOSE_OUTPUT)THEN
	  WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'End cont loop'
	END IF
...
!
	...
	IF(VERBOSE_OUTPUT)THEN
	  WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'two'
	END IF
!
! 
!
...
!
	...
	IF(VERBOSE_OUTPUT)THEN
	  WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'chg_1'
	END IF
!
	DO ID=1,NUM_IONS-1
	...
	END DO
	IF(VERBOSE_OUTPUT)THEN
	  WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'chg_2'
	END IF
! 
!
...
!
	...
	1             TIME_SEQ_NO,NUM_BNDS,ND,NT)
	  IF(VERBOSE_OUTPUT)THEN
	    WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'SN_DDT'
	  END IF
	ELSE
...
!
	...
	1                       TIME_SEQ_NO,DIAG_INDX,NUM_BNDS,NT,ND)
	  IF(VERBOSE_OUTPUT)THEN
	    WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'SN_ADD'
	  END IF
	ELSE
	  dE_WORK=0.0D0
...
!
	IF(SN_MODEL .AND. INCL_RADIOACTIVE_DECAY)THEN
	  ...
	  IF(VERBOSE_OUTPUT)THEN
	    WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'SN_RAD'
	  END IF
	END IF
!
...
!
	...
	IF(VERBOSE_OUTPUT)THEN
	  WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(NT,DIAG_INDX,DPTH_INDX),'Artificial heat'
	END IF
!
