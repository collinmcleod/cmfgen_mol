...
!
	...
	IMPLICIT NONE
!
! Altered 16-Nov-2022 : changed the way POPS are written to file on last iteration for molecules
! Altered 07-Jun-2022 : removed calculation of free-free opacity for molecules
...
!
	...
	IF (F_TO_S_RD_ERROR) THEN
	   WRITE(6,*) ' '
	   WRITE(6,*) 'There are errors reading in the super levels'
	   WRITE(6,*) 'These need to be fixed before the code'
...
!
!**********************************************************************************************
!  Running the gamma-ray code for SNe. This calculation only needs to be run once, since it
!  (generally) only depends on the total electron density.
!
!**********************************************************************************************
!
	...
	1	   INSTANTANEOUS_ENERGY_DEPOSITION,SN_AGE_DAYS)
	   ...
	   CALL TUNE(3,' ')
	ELSE IF(GAMRAY_TRANS .EQ. 'RAD_TRANS')THEN
	   WRITE(6,'(A)')'Using previously computed GAMRAY_ENERGY_DEP file'
	END IF
!
!**************************************************************************
!**************************************************************************
...
!
	...
	1	  COMPUTE_BA,FIXED_T,LST_ITERATION,DST,DEND,IS_MOLECULE(SPECIES_LNK(ID)))
!
...
!
	IF(TREAT_NON_THERMAL_ELECTRONS)THEN
	  ...
	  CALL SE_BA_NON_THERM_V3(dE_RAD_DECAY,dE_SHOCK_POWER,COMPUTE_BA,NT,ND,1.0_LDP) !DEC_NRG_SCL_FAC)
!	    CALL SE_BA_NON_THERM_V3(dE_RAD_DECAY,COMPUTE_BA,NT,ND,DEC_NRG_SCL_FAC)
...
!
	...
	1	 ID,TMP_STRING,OMEGA_GEN_V4,ED,T,ND,IS_MOLECULE(SPECIES_LNK(ID)))
	    END IF
	  END DO
...
!
	...
	1            ') at outer boundary is:',CHI_CONT(1)*R(1)/MAX(T1-1.0_LDP,1.0_LDP)
...
!
! 
! Add Carbon Monoxide LTE cooling to electron energy balance eqn.
!
!	IF (INCL_MOL_RXN) THEN
!	   DO I=1,ND
!	     STEQ_T_EHB(I)=STEQ_T_EHB(I)-COMI_NET_COOLING(I) !check sign
!	   END DO
!	END IF
!
! 
...
!
	IF (INCL_MOL_RXN) THEN
	   CALL TUNE(1,'MOL_RXN')
	   CALL STEQ_BA_MOL_RXN_V3(POPS,NT,ND,DIAG_INDX,COMPUTE_BA)
	   IF (LST_ITERATION .AND. VERBOSE_OUTPUT) THEN
...
!
! Write out COMI cooling (every iteration)
! I write the temp, COMI line cooling rate, radioactive decay heating rate, and co population 
! for each depth point
!
!	IF (INCL_MOL_RXN) THEN
!	   INQUIRE(UNIT=LU_CO,EXIST=FILE_OPEN)
!	   IF (FILE_OPEN) THEN
!	      OPEN(LU_CO,ACTION='READWRITE')
!	      DO
!		 READ(LU_CO,'(A)',IOSTAT=IOS) STRING
!		 IF(IOS .NE. 0) THEN
!		    EXIT
!		 END IF
!		 IF(LEN(TRIM(ADJUSTL(STRING))) .LE. 5 .AND. LEN(TRIM(ADJUSTL(STRING))) .GE. 1) THEN
!		    READ(STRING,*) CO_COUNTER
!		 END IF
!	      END DO
!	      CLOSE(LU_CO)
!	      OPEN(LU_CO,POSITION='APPEND',ACTION='WRITE')
!	   ELSE
!	      OPEN(LU_CO,FILE='CO_COOL_SCR',ACTION='WRITE')
!	   END IF
!
!	   WRITE(*,*) 'CO_COUNTER= ',CO_COUNTER
!
!	   DO ISPEC=1,NUM_SPECIES
!	      IF(TRIM(ADJUSTL(SPECIES(ISPEC))) .EQ. 'COMOL') THEN
!		 CO_VARIABLE=ATM(SPECIES_BEG_ID(ISPEC))%EQXzV
!	      END IF
!	   END DO
!
!	   DO L=CO_COUNTER+1,MAIN_COUNTER
!
!	      WRITE(LU_CO,*)
!	      WRITE(LU_CO,'(I4)') L
!	      WRITE(LU_CO,*)
!	      DO I=1,ND
!!		 WRITE(LU_CO,'(I4,ES12.4,ES12.4,ES12.4,ES12.4)') I,T(I),COMI_NET_COOLING(I),dE_RAD_DECAY(I),POPS(CO_VARIABLE,I)
!	      END DO
!	      WRITE(LU_CO,*)
!	      
!	   END DO
...
!	   CLOSE(LU_CO)
!
!	   WRITE(*,*) 'Wrote CO cooling to SCR file'
!	END IF
!
	IF(LST_ITERATION .AND. .NOT. USE_FIXED_J)THEN
!
	...
	1	   ATM(ID)%MOL_DRXzV, ATM(ID)%MOL_CRXzV, SE(ID)%STEQ_ADV,
...
!
	    IF (INCL_MOL_RXN) THEN
	       CALL WR_COOLING_TERM(COMI_NET_COOLING,TA,TB,L_TRUE,'Carbon Monoxide Cooling',LS,ND,LU_REC_CHK)
	    END IF
!
...
!
	...
	1    'Dielectronic and Implicit Recombination Line Emission',LU_FLUX)
	  ...
	  IF(SUM(DJDt_FLUX) .NE. 0.0_LDP)CALL WRITV(DJDt_FLUX,ND,'Flux arising from Dr^3J/Dt term',LU_FLUX)
