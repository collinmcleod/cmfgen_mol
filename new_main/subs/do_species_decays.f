...
!
	LUER=ERROR_LU()
	IF(NUM_DECAY_PATHS .NE. 4)THEN
	  WRITE(LUER,*)'Error in UPDATE_POPS_FOR_DECAYS'
	  WRITE(LUER,*)'Ivalid number of transactions'
	  STOP
	END IF
!
	DO IS=1,NUM_ISOTOPES
	  ISO(IS)%OLD_POP_DECAY=ISO(IS)%OLD_POP
	  WRITE(6,*)IS,ISO(IS)%SPECIES,ISO(IS)%OLD_POP(1)
	END DO
	RADIOACTIVE_DECAY_ENERGY=0.0D0
!
	WRITE(6,*)'Delta T=',DELTA_T
	DO IN=1,NUM_DECAY_PATHS
	  IF(NUC(IN)%SPECIES.EQ. 'NICK')THEN
	    IS=NUC(IN)%LNK_TO_ISO
	    VEC1=EXP(-NUC(IN)%DECAY_CONST*DELTA_T)
	    DO JN=1,NUM_DECAY_PATHS
	      IF(NUC(JN)%SPECIES .EQ.  'COB' .AND. NUC(JN)%BARYON_NUMBER .EQ.  NUC(IN)%BARYON_NUMBER)THEN
	        WRITE(6,*)'FOund:',IN,JN
...
!
	...
	DO IS=1,NUM_ISOTOPES
	  ...
	  WRITE(6,*)IS,ISO(IS)%SPECIES,ISO(IS)%OLD_POP(1)
	  WRITE(6,*)IP
	  WRITE(6,*)IP,PAR(IP)%OLD_POP(1),PAR(IP)%OLD_POP_DECAY(1)
	END DO
!
	RETURN
	END
