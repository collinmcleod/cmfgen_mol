...
!                
	...
	END DO
!
	IF(K .EQ. 29 .AND. DIAG_BAND)THEN
	  OPEN(UNIT=96,FILE='BA_ASCI_N_D29',STATUS='UNKNOWN')
	    CALL WR2D_MA(POPS(1,K),NT,1,'POPS_D1',96)
	    CALL WR2D_MA(STEQ_VEC,NT,1,'STEQ_VEC_D1',96)
	    CALL WR2D_MA(C_MAT,NT,NT,'C_MAT_D1',96)
	  CLOSE(UNIT=96)
	END IF
	IF(K .EQ. 84 .AND. DIAG_BAND)THEN
	  OPEN(UNIT=96,FILE='BA_ASCI_N_D84',STATUS='UNKNOWN')
	    CALL WR2D_MA(POPS(1,K),NT,1,'POPS_D1',96)
	    CALL WR2D_MA(STEQ_VEC,NT,1,'STEQ_VEC_D1',96)
	    CALL WR2D_MA(C_MAT,NT,NT,'C_MAT_D1',96)
	  CLOSE(UNIT=96)
