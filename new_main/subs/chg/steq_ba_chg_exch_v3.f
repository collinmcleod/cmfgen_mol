...
!
	...
	1              COEF_CHG(J,4)*T1*TVAL/(1.0_LDP+T1)
	      ...
	      ELSE IF (TYPE_CHG(J) .EQ. 4)THEN
 		 T1=EXP(-TVAL*1.0E+04/COEF_CHG(J,3))
		 ALPHA_REC=COEF_CHG(J,1)*(TVAL**COEF_CHG(J,2))*T1
		 T1=EXP(-TVAL*1.0E+04/COEF_CHG(J,6))
		 ALPHA_REC=ALPHA_REC + (COEF_CHG(J,4)*(TVAL**COEF_CHG(J,5))*T1)
		 dlnALPHA_RECdlnT=COEF_CHG(J,2) + COEF_CHG(J,5) - TVAL*1.0E+04/(COEF_CHG(J,3)+COEF_CHG(J,6))
	      ELSE
	        LU_ER=ERROR_LU()
