...
!
	...
	DO IS=1,NUM_ISOTOPES
	  ...
	  END IF
	  READ(LU,'(A)')STRING
	  WRITE(6,*)IS, ISO(IS)%SPECIES, ISO(IS)%MASS, ISO(IS)%BARYON_NUMBER
	END DO
...
!
	  ...
	  STRING=STRING(L+1:) 
	  IF(IN .NE. NUM_DECAY_PATHS)READ(LU,'(A)')STRING
	END DO
	CLOSE(LU)
...
!
	DO IP=1,NUM_PARENTS
	  ALLOCATE (PAR(IP)%OLD_POP(ND))
	  ALLOCATE (PAR(IP)%OLD_POP_DECAY(ND))
	END DO
!
	DO IS=1,NUM_ISOTOPES
