...
!	    WRITE(6,*)'Found one step nuclear reaction chain:',IN
	    ...
	    IF(INSTANTANEOUS_ENERGY_DEPOSITION)THEN 
	...
	1            ISO(IS)%OLD_POP_DECAY*NUC(IN)%KINETIC_PER_DECAY*NUC(IN)%DECAY_CONST
	      CUR_RAD_SUM=SUM(RADIOACTIVE_DECAY_ENERGY)
	      CUR_EK_SUM=SUM(KINETIC_DECAY_ENERGY)
...
!	        WRITE(6,*)'Found nuclear reaction chain:',IN,JN
	...
	END DO
!
	IF(DELTA_T .NE. 0.0D0 .AND. .NOT. INSTANTANEOUS_ENERGY_DEPOSITION)THEN
