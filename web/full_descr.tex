...
\begin{mylist}{longest label}
\item[FIX\_ALL\_SPEC]    When true all species are held fix. This option can be used with [UNFIX\_XzV] to a single, or multiple ionization stage, to be updated by the CMFGEN calculaton. XzV. Mainly used when adding new species and/or ionization stages. Default is FALSE.

\item[UNFIX\_XzV]    Allow a single ionization stage to be removed from the influence of the [FIX\_ALL\_SPC] option.
Set to 0 to UNFIX the ionization stage
...
\indent
GENSCOOL\_SORT: Only the top rates are printed: Sorted using depths 1, 11, 21 etc. Thus, at depth 20, for example,
rates will not be in order.
