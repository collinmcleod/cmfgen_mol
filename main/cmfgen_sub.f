...
C 
C
C BA: Equations of Statistical Equilibrium Variation matrix:
C Equation,variable,depth of variable,depth of equation
C On ALPHA machines, it is preferable to allocate the BA array.
C this saves stack space.
C	REAL*8 BA(NT,NT,NUM_BNDS,ND)
C
	REAL*8, ALLOCATABLE :: BA(:,:,:,:)
C
C STEQ: Statistical Equilibrium Equations:
C       Equation,dep of equ.
C
	REAL*8 STEQ(NT,ND)
	REAL*8 SOL(NT,ND)		!Temp. stor. area for ST. EQ.
	INTEGER*4 DST,DEND
C
C Similar to BA and STEQ but equilibrium equations for EACH ion.
C
	REAL*8, ALLOCATABLE :: BAION(:,:,:,:)
	REAL*8, ALLOCATABLE :: STEQION(:,:)
C
...
C
	...
	REAL*8 TSTAR,S1,IC,REPA,EPS,MAXCH
...
C
	REAL*8 TGREY(ND),GREY_PAR,T_INIT_TAU
...
C
	REAL*8 QFV_R(NT,ND),QFV_P(NT,ND)
...
C
	...
	FORMFEED=' '//CHAR(I)
        CNT_FIX_BA=0
!
	ALLOCATE (BAION(NION,NT,NUM_BNDS,ND),STAT=IOS)
	IF(IOS .EQ. 0)ALLOCATE (BA(NT,NT,NUM_BNDS,ND),STAT=IOS)
	IF(IOS .EQ. 0)ALLOCATE (STEQION(NION,ND),STAT=IOS)
	IF(IOS .NE. 0)THEN
	  WRITE(LUER,*)'Unable to allocate memory for BA, BAION or ',
	1                 'STEQION arrays in CMFGEN_SUB'
	  STOP
	END IF
!
	T1=10.0/(NLF-1)
...
!
	...
	1           'Scale factor for 2nd Beta V law')
	    V_REF_RADIUS=RP
	    CALL RD_STORE_DBLE(V_REF_RADIUS,'V_REF_RAD',L_FALSE,
	1           'Reference radius for velocity law')
!
	    NBND_INS=1          !Old default
	    CONS_FOR_R_GRID=-1.0D0
...
!
	...
	1            'Iterate on Net-rates for weak lines?')
C
...
C
	CALL RD_CHG_EXCH(LUIN,INCL_CHG_EXCH)
!
...
C
	WRITE(LUER,*)' '
	WRITE(LUER,'(A,T40,I5)')' Number of line frequencies is:',N_LINE_FREQ
...
C
	...
	1   ,RP2,RN2,VRP2,VINF2,EPPS2,GAMMA2,ND,TA,TB,TC)
	  ELSE IF(VELTYPE .EQ. 3 .OR. VELTYPE .EQ. 6)THEN
	    CALL STARPCYG_V4(R,V,SIGMA,RMAX,RP,
	1             SCL_HT,VCORE,VPHOT,VINF1,V_BETA1,V_EPPS1,
	1             VINF2,V_BETA2,V_EPPS2,V_REF_RADIUS,
...
C
	     DO ID=1,NUM_IONS-1
	       CALL SET_TWO_PHOT(ION_ID(ID), ATM(ID)%XzVLTE, ATM(ID)%NXzV,
...
C
	    IF( MAIN_COUNTER .EQ. 1)THEN
	      DO I=1,ND
	        ZNET(I)=T(I)		!Save original T for use when
	      END DO                    !correcting T towards TGREY.
	    ELSE IF(T(1) .NE. ZNET(1))THEN
	      WRITE(LUER,*)'Error in CMFGEN --- TSAVE (i.e. ZNET)'//
	1               ' corrupted in initial T iteration loop'
	      STOP
	    END IF
 	    T2=0.0
	    DO I=1,ND
	      ...
	      END IF
	      IF(TA(I) .LT. 0.1*GREY_PAR .OR. I .EQ. 1)T1=0.0
	      T3=ABS( T1*(TGREY(I)-T(I)) )
	      T(I)=T1*TGREY(I)+(1.0-T1)*ZNET(I)
	      T2=MAX(T3/T(I),T2)
	    END DO
	    WRITE(LUER,'('' Largest correction to T in Initialization loop '//
...
C
	...
	IF(NUM_ITS_TO_DO .EQ. 0)THEN
	  IF(RDINSOL)THEN
	    CALL LOCSOLUT(POPS,STEQ,TA,30,NT,ND)
C
	    DO ID=1,NUM_IONS-1
	      CALL POPTOION(POPS, ATM(ID)%XzV, ATM(ID)%DXzV, ED,T,
	1            ATM(ID)%EQXzV, ATM(ID)%NXzV, NT,ND, 
...
C 
C
C Read in BA and STEQ arrays
C
	CALL READBA(BA,STEQ,LU_BA,NT*NT*NUM_BNDS*ND,
	1                   NT*ND,NLBEGIN,COMPUTE_BA,CHK,'BAMAT')
	IF(CHK)THEN
	  CALL READBA(BAION,STEQION,LU_BA,NION*NT*NUM_BNDS*ND,
	1                     NION*ND,NLBEGIN,COMPUTE_BA,CHK,'BAION')
	END IF
...
!
	    ...
	    CALL WRITE_DIRECT_INFO_V3(ND,I,'20-Aug-2000','EDDFACTOR',LU_EDD)
...
C
	...
	DTDR=0.0D0
	SECTION='DTDR'
	IF(IMPURITY_CODE .OR. FLUX_CAL_ONLY)THEN
...
C 
C
C Check to see if continuum calculation is necessary (ie we are not
C part way through an iteration)
C
		IF(NLBEGIN .LE. 0)THEN
C
C Zero STEQ and BA arrays.
C
	CALL DP_ZERO(STEQ,NT*ND)
	CALL DP_ZERO(BA,NT*NT*NUM_BNDS*ND)
	CALL DP_ZERO(STEQION,NION*ND)
	CALL DP_ZERO(BAION,NION*NT*NUM_BNDS*ND)
C
	CALL DP_ZERO(BA_PAR,NT*NT*ND)
	CALL DP_ZERO(BAION_PAR,NION*NT*ND)
C 
C
...
C
	  ...
	  DO ID=1,NUM_IONS-1
	    IF(ATM(ID)%XzV_PRES)THEN
	      ...
	      CALL STEQ_MULTI_V4(BA,STEQ, CNM,DCNM,ED,T,
	...
	1         ID,TMP_STRING,OMEGA_GEN_V3,
	1         ATM(ID)%EQXzV,EQ_SPECIES(SPECIES_LNK(ID)),NT,NUM_BNDS,ND,
	1         BAION,STEQION, ATM(ID)%EQXzVBAL,NION,DST,DEND)
	    END IF
	  END DO
	  CALL TUNE(2,'STEQ')
C        
...
C
	  CALL STEQNE(BA,STEQ,ED,EQNE,NT,NUM_BNDS,ND,DST,DEND)
C
	END DO		!K=1,ND  - DST,DEND
C
	IF(LST_ITERATION)
	1     CALL WR2D(STEQ,NT,ND,'STEQ ARRAY- Collisional Terms',19)
C 
C
...
C
	IF(.NOT. COMPUTE_BA .AND. .NOT. FLUX_CAL_ONLY)THEN
	  CALL READBA(BA,SOL,LU_BA,NT*NT*NUM_BNDS*ND,NT*ND,I,
	1        IOS,CHK,'BAMAT')
	  IF(.NOT. CHK)THEN
	...
	1             'before continuum loop'
	    STOP
	  END IF
	  CALL READBA(BAION,SOL,LU_BA,NION*NT*NUM_BNDS*ND,NION*ND,
	1             I,IOS,CHK,'BAION')
	  IF(.NOT. CHK)THEN
	    WRITE(LUER,*)'Major Error - cant read BAION File'
	    WRITE(LUER,*)'Previously read successfully - '//
	1             'before continuum loop'
	    STOP
	  END IF
...
C
	  DO K=1,ND
	    STEQ(NL,K)=STEQ(NL,K)+EINA(1)*NUST(K)*ZNET(K)
	    STEQ(NT,K)=STEQ(NT,K)-T3*NUST(K)*ZNET(K)
...
C
	  IF(EQBAL .NE. 0)THEN
	    DO K=1,ND
	      STEQION(EQBAL,K)=STEQION(EQBAL,K)+EINA(1)*NUST(K)*ZNET(K)
	    END DO
	  END IF
	  IF(EQION .LT. EQSPEC)THEN
	    DO K=1,ND
	      STEQ(EQION,K)=STEQ(EQION,K)-EINA(1)*NUST(K)*ZNET(K)
	    END DO
	  END IF
C 
C
...
C
	    DO K=1,ND
	      L=GET_DIAG(K)
	      BA(NL,NL,L,K)=BA(NL,NL,L,K)+EINA(1)*VB(K)
	      BA(NL,EQION,L,K)=BA(NL,EQION,L,K)+EINA(1)*VC(K)/DION(K)
	      BA(NL,NT-1,L,K)=BA(NL,NT-1,L,K)+EINA(1)*VC(K)/ED(K)
	      BA(NL,NT,L,K)=BA(NL,NT,L,K)-
	1          EINA(1)*VC(K)*(1.5D0+HDKT*EDGEDIE(ML)/T(K))/T(K) +
	1          EINA(1)*VB(K)*POPS(NL,K)*dL_RAT_dT(K,1)/L_STAR_RATIO(K,1)
C
	      BA(NT,NL,L,K)=BA(NT,NL,L,K)-T3*VB(K)
	      BA(NT,EQION,L,K)=BA(NT,EQION,L,K)-T3*VC(K)/DION(K)
	      BA(NT,NT-1,L,K)=BA(NT,NT-1,L,K)-T3*VC(K)/ED(K)
	      BA(NT,NT,L,K)=BA(NT,NT,L,K)+
...
C
	    IF(EQBAL .NE. 0)THEN
	      DO K=1,ND
	        L=GET_DIAG(K)
	        BAION(EQBAL,NL,L,K)=BAION(EQBAL,NL,L,K)+EINA(1)*VB(K)
	        BAION(EQBAL,EQION,L,K)=BAION(EQBAL,EQION,L,K) +
	1                    EINA(1)*VC(K)/DION(K)
	        BAION(EQBAL,NT-1,L,K)=BAION(EQBAL,NT-1,L,K) +
	1                    EINA(1)*VC(K)/ED(K)
	        BAION(EQBAL,NT,L,K)=BAION(EQBAL,NT,L,K) -
	1          EINA(1)*VC(K)*(1.5D0+HDKT*EDGEDIE(ML)/T(K))/T(K) +
	1          EINA(1)*VB(K)*POPS(NL,K)*dL_RAT_dT(K,1)/L_STAR_RATIO(K,1)
	      END DO
	    END IF
...
C
	    IF(EQION .LT. EQSPEC)THEN
	      DO K=1,ND
	        L=GET_DIAG(K)
	        BA(EQION,NL,L,K)=BA(EQION,NL,L,K)-EINA(1)*VB(K)
	        BA(EQION,EQION,L,K)=BA(EQION,EQION,L,K) -
	1                    EINA(1)*VC(K)/DION(K)
	        BA(EQION,NT-1,L,K)=BA(EQION,NT-1,L,K) -
	1                    EINA(1)*VC(K)/ED(K)
	        BA(EQION,NT,L,K)=BA(EQION,NT,L,K) +
	1           EINA(1)*VC(K)*(1.5D0+HDKT*EDGEDIE(ML)/T(K))/T(K) -
	1           EINA(1)*VB(K)*POPS(NL,K)*dL_RAT_dT(K,1)/L_STAR_RATIO(K,1)
	      END DO
	    END IF
...
C
	    ...
	    DO L=1,ND	  	  		  	!S.E. equation depth
	      ...
	      DO K=BNDST(L),BNDEND(L)	  		!Variable depth.
	        LS=BND_TO_FULL(K,L)
   	        DO J=1,NT	 	   		!Variable
	          IF( DABS(VJ(J,K,L)*1.0D+06*POPS(J,LS)) .GE. RJ(L))THEN
	            BA(NL,J,K,L)=BA(NL,J,K,L) - T1*VJ(J,K,L)
	            BA(NT,J,K,L)=BA(NT,J,K,L) + T2*VJ(J,K,L)
	          END IF
	        END DO
	      END DO
	    END DO
	      ...
	      DO L=1,ND	  	  		  	!S.E. equation depth
	        ...
	        DO K=BNDST(L),BNDEND(L)	  		!Variable depth.
	          LS=BND_TO_FULL(K,L)
   	          DO J=1,NT	 	   		!Variable
	            IF( DABS(VJ(J,K,L)*1.0D+06*POPS(J,LS)) .GE. RJ(L))THEN
	              BAION(EQBAL,J,K,L)=BAION(EQBAL,J,K,L) - T1*VJ(J,K,L)
	            END IF
	          END DO
	        END DO
	      END DO
	      ...
	      DO L=1,ND	  	  		  	!S.E. equation depth
	        ...
	        DO K=BNDST(L),BNDEND(L)	  		!Variable depth.
	          LS=BND_TO_FULL(K,L)
   	          DO J=1,NT	 	   		!Variable
	            IF( DABS(VJ(J,K,L)*1.0D+06*POPS(J,LS)) .GE. RJ(L))THEN
	              BA(EQION,J,K,L)=BA(EQION,J,K,L) + T1*VJ(J,K,L)
	            END IF
	          END DO
	        END DO
	      END DO
...
!
	IF(USE_OLDJ_FOR_ES .AND. .NOT. RD_COHERENT_ES .AND. COHERENT_ES)THEN
	  COHERENT_ES=RD_COHERENT_ES
	  I=ND*NT*NT*NUM_BNDS
	  CALL COMP_J_CONV_V2(BA,I,NU,TEXT,NDEXT,NCF,LUIN,'OLD_J_FILE',
...
!
	...
	1                        (VTURB/12.85)**2 )/2.998D+05
	    ...
	    DO I=1,ND
	     IF( ABS(CHIL_MAT(I,SIM_INDX))*T2/ESEC(I) .GT. 0.01)
...
C
	  CALL TUNE(1,'VAR_ZERO')
	  IF(.NOT. WEAK_LINE(SIM_INDX))THEN
	    I=TX_OFFSET+1
	    DO WHILE( VAR_LEV_ID(I) .NE. SIM_NL(SIM_INDX))
	      I=I+1
	      IF(I .GT. NM)THEN
C
...
C
	    I=TX_OFFSET+1
	    DO WHILE( VAR_LEV_ID(I) .NE. SIM_NUP(SIM_INDX))
	      I=I+1
	      IF(I .GT. NM)THEN
C
C This variable currently does not have a storage location. Therefore
...
C
	  DO K=1,ND
	    STEQ(NT,K)=STEQ(NT,K)+ FQW(ML)*( 
	1       (CHI_CONT(K)-ESEC(K))*RJ(K) - ETA_CONT(K) )
	  END DO
	  IF(.NOT. COHERENT_ES)THEN
	    STEQ(NT,:)=STEQ(NT,:)+FQW(ML)*ESEC(:)*(RJ(:)-RJ_ES(:))
	  END IF
C
...
C
	IF(FINAL_CONSTANT_CROSS)THEN
	  QFV_R(:,:)=0.0D0		!NT,ND
	  QFV_P(:,:)=0.0D0
	  QFVION_R(:,:)=0.0D0		!NION,ND
	  QFVION_P(:,:)=0.0D0		!NION,ND
	END IF
C
	IF(FINAL_CONSTANT_CROSS)THEN
	  ...
	  DO ID=1,NUM_IONS-1
	    IF(ATM(ID)%XzV_PRES)THEN
	      DO J=1,ATM(ID)%N_XzV_PHOT
	        CALL EVALSE_QWVJ_V5(STEQ,QFV_R,QFV_P,
	1         ATM(ID)%WSXzV(1,1,J), ATM(ID)%XzV, ATM(ID)%XzVLTE,
	1         ATM(ID)%NXzV, ATM(ID)%EQXzV, ATM(ID)%XzV_ION_LEV_ID(J),
	1         ATM(ID+1)%XzV, ATM(ID+1)%XzVLTE, ATM(ID+1)%NXzV, ATM(ID+1)%EQXzV,
	1         JREC,JPHOT,EQ_SPECIES(SPECIES_LNK(ID)),NT,ND,
	1         STEQION,QFVION_R,QFVION_P,ATM(ID)%EQXzVBAL,NION)
	      END DO
	    END IF
	  END DO
...
C
	IF(XRAYS .AND. FINAL_CONSTANT_CROSS)THEN
	  DO ID=1,NUM_IONS-1
	    IF(ATM(ID)%XzV_PRES .AND. ATM(ID+1)%XzV_PRES)THEN
	      CALL EVALSE_X_QWVJ_V3(STEQ,QFV_R,QFV_P, ATM(ID)%WSE_X_XzV,
	1          ATM(ID)%XzV,   ATM(ID)%XzVLTE,   ATM(ID)%NXzV,
	1          ATM(ID+1)%XzV, ATM(ID+1)%XzVLTE, ATM(ID+1)%NXzV,
	1          JREC,JPHOT, ATM(ID)%EQXzV,
	1          ATM(ID+2)%EQXzV,EQ_SPECIES(SPECIES_LNK(ID)),NT,ND,
	1          STEQION,QFVION_R,QFVION_P,
	1          ATM(ID)%EQXzVBAL, ATM(ID+1)%EQXzVBAL,NION)
	    END IF
	  END DO
	END IF
C
	...
	DO SIM_INDX=1,MAX_SIM
	  IF( END_RES_ZONE(SIM_INDX) )THEN
	    ...
	    DO K=1,ND					!Equation depth
	      ...
	      STEQ(NUP,K)=STEQ(NUP,K) - T2/T1
	      STEQ(NL,K)=STEQ(NL,K) + T2/T1
	      STEQ(NT,K)=STEQ(NT,K) - T2*T3
	    END DO
	  END IF    
	END DO
	CALL TUNE(2,'EVAL_ZNET')
C
C                                                                    
C
C Allow for the variation of the continuous radiation field.
...
C
	    IF(FINAL_CONSTANT_CROSS)THEN
	      DO ID=1,NUM_IONS-1
	        IF(ATM(ID)%XzV_PRES)THEN
	          DO J=1,ATM(ID)%N_XzV_PHOT
	            CALL VSEBYJ_MULTI_V4(
	1             BA_PAR,ATM(ID)%WSXzV(1,1,J), ATM(ID)%dWSXzVdT(1,1,J),
	1             ATM(ID)%XzV, ATM(ID)%XzVLTE, ATM(ID)%dlnXzVLTE_dlnT, 
	1             ATM(ID)%NXzV,
	1             ATM(ID+1)%XzV, ATM(ID+1)%XzVLTE,
	...
	1             ATM(ID)%EQXzV, ATM(ID+1)%EQXzV,
	1             EQ_SPECIES(SPECIES_LNK(ID)),NT,IONE,ND,
	1             BAION_PAR, ATM(ID)%EQXzVBAL,NION,DST,DEND)
	          END DO
	        END IF
	      END DO
	    END IF
...
C
	    IF(XRAYS .AND. FINAL_CONSTANT_CROSS)THEN
	      DO ID=1,NUM_IONS-1
	        IF(ATM(ID)%XzV_PRES .AND. ATM(ID+1)%XzV_PRES)THEN
	          J=EQ_SPECIES(SPECIES_LNK(ID))
	          CALL VSEBYJ_X_V4(BA_PAR,ATM(ID)%WSE_X_XzV,
	1             ATM(ID)%XzV, ATM(ID)%XzVLTE, ATM(ID)%dlnXzVLTE_dlnT, ATM(ID)%NXzV,
	1             ATM(ID+1)%XzV_F, ATM(ID+1)%XzVLTE_F, ATM(ID+1)%EDGEXzV_F,
	1             ATM(ID+1)%NXzV_F, ATM(ID+1)%DXzV,ED,T,JREC,dJRECdt,JPHOT,
	1             ATM(ID)%EQXzV,ATM(ID+2)%EQXzV,J,NT,IONE,ND,
	1             BAION_PAR, ATM(ID)%EQXzVBAL, ATM(ID+1)%EQXzVBAL,
	1             NION,DST,DEND)
	        END IF
	      END DO
	    END IF		!End X-rays
...
C
	    IF( .NOT. LAMBDA_ITERATION)THEN
	      CALL BA_UPDATE_V4(BA,BA_PAR,QFV_R,QFV_P,VJ,VCHI,VETA,
	1             CHI_CONT,ESEC,T,POPS,RJ,NU(ML),FQW(ML),
	1             COMPUTE_NEW_CROSS,FINAL_CONSTANT_CROSS,
	1             BA_CHK_FAC,NT,NUM_BNDS,ND,DST,DEND)
	      CALL BAION_UPDATE_V4(BAION,BAION_PAR,QFVION_R,QFVION_P,VJ,
	1             T,POPS,RJ,NU(ML),FQW(ML),
	1             COMPUTE_NEW_CROSS,FINAL_CONSTANT_CROSS,
...
C
	  IF( MOD(FREQ_INDX,N_PAR) .EQ. 0 .OR. FREQ_INDX .EQ. NCF )THEN
	    CALL ADD_PAR_TO_FULL(BA,BA_PAR,NT,NT,NUM_BNDS,ND)
	    CALL ADD_PAR_TO_FULL(BAION,BAION_PAR,NION,NT,NUM_BNDS,ND)
	  END IF
C
...
!
	...
	1              L_STAR_RATIO(K,SIM_INDX)*OPAC_FAC*POPS(NUP,K)*VB(K)
	        L=GET_DIAG(K)
	        BA(NUP,NUP,L,K)=BA(NUP,NUP,L,K)-dRATE_dUP
	        BA(NUP,NL,L,K)=BA(NUP,NL,L,K)-dRATE_dLOW
	        BA(NL,NUP,L,K)=BA(NL,NUP,L,K)+dRATE_dUP
	        BA(NL,NL,L,K)=BA(NL,NL,L,K)+dRATE_dLOW
	        BA(NT,NL,L,K)=BA(NT,NL,L,K)-T4*dRATE_dLOW
	        BA(NT,NUP,L,K)=BA(NT,NUP,L,K)-T4*dRATE_dUP
	      END DO
...
C
	      ...
	      DO L=1,ND					!Equation depth
	        ...
	        K=GET_DIAG(L)
	        BA(NL,NUP,K,L)=BA(NL,NUP,K,L) +
	1           EINA(SIM_INDX)*U_STAR_RATIO(L,SIM_INDX)*
	1             ZNET_SIM(L,SIM_INDX)
	        BA(NUP,NUP,K,L)=BA(NUP,NUP,K,L) -
	1           EINA(SIM_INDX)*U_STAR_RATIO(L,SIM_INDX)*
	1            ZNET_SIM(L,SIM_INDX)
	        BA(NT,NUP,K,L)=BA(NT,NUP,K,L) -
	1           T3*ETAL_MAT(L,SIM_INDX)*ZNET_SIM(L,SIM_INDX)/POPS(NUP,L)
		T4=ABS(ZNET_SIM(L,SIM_INDX)-1.0D0)
	        IF(T4 .GT. ZNET_VAR_LIMIT)THEN
	          DO K=1,NUM_BNDS			!Variable depth
	            DO J=1,NT
	              BA(NL,J,K,L)=BA(NL,J,K,L)+dZ_POPS(J,K,L)*T1
	              BA(NUP,J,K,L)=BA(NUP,J,K,L)-dZ_POPS(J,K,L)*T1
	              BA(NT,J,K,L)=BA(NT,J,K,L)-dZ_POPS(J,K,L)*
...
C
	    IF( NU(ML) .LT. NU(LINE_END_INDX_IN_NU(L))*T1 .AND.
	1                                 .NOT. WEAK_LINE(SIM_INDX))THEN
	      NL=LOW_POINTER(SIM_INDX)
	      VAR_IN_USE_CNT(NL)=VAR_IN_USE_CNT(NL)-1
	      IF(VAR_IN_USE_CNT(NL) .EQ. 0)THEN
	        I=ND*ND
	        CALL DP_ZERO(TX(1,1,NL),I)
	        I=ND*(ND-1)
	        CALL DP_ZERO(TVX(1,1,NL),I)
	        DO I=1,SIM_INDX
	          DO K=1,ND
	            DO L=BNDST(K),BNDEND(K)
	              dZ(NL,L,K,I)=0.0D0
	            END DO
	          END DO
	        END DO
...
C
	      ...
	      IF(VAR_IN_USE_CNT(NUP) .EQ. 0)THEN
	        I=ND*ND
	        CALL DP_ZERO(TX(1,1,NUP),I)
	        I=ND*(ND-1)
	        CALL DP_ZERO(TVX(1,1,NUP),I)
	        DO I=1,SIM_INDX
	          DO K=1,ND
	            DO L=BNDST(K),BNDEND(K)
	              dZ(NUP,L,K,I)=0.0D0
...
C 
C
	CALL STEQ_BA_TWO_PHOT_RATE(STEQ,BA,POPS,NT,ND,NUM_BNDS,
...
C
	DO ID=1,NUM_IONS-1
	  IF(ATM(ID)%XzV_PRES)THEN
	    CALL SET_CHG_EXCH_V2(ION_ID(ID), ATM(ID)%XzVLEVNAME_F,
	1       ATM(ID)%EDGEXzV_F,  ATM(ID)%GXzV_F,
	1       ATM(ID)%F_TO_S_XzV, ATM(ID)%NXzV_F, ATM(ID)%NXzV,
	1       ND,T, ATM(ID)%ZXzV, ATM(ID)%GIONXzV_F,
	1       ATM(ID)%EQXzV, ATM(ID)%EQXzVBAL, EQ_SPECIES(SPECIES_LNK(ID)))
	  END IF
	END DO
!
	CALL VERIFY_CHG_EXCH()
	CALL STEQ_BA_CHG_EXCH(BA,STEQ,BAION,STEQION,
	1           POPS,T,NT,ND,NUM_BNDS,NION,DIAG_INDX,COMPUTE_BA)
C
	DO ID=1,NUM_IONS-1
	  CALL EVAL_CHG_RATES(ATM(ID)%CHG_PRXzV, ATM(ID)%CHG_RRXzV,
...
C
C Write pointer file and store BA and STEQ matrices.
C
	IF(COMPUTE_BA
	1          .AND. WRBAMAT 
	1          .AND. .NOT. FLUX_CAL_ONLY 
	1          .AND. .NOT. LAMBDA_ITERATION)THEN
	  CALL STOREBA(BA,STEQ,LU_BA,NT*NT*NUM_BNDS*ND,NT*ND,
	1              NLBEGIN,COMPUTE_BA,'BAMAT')
	  CALL STOREBA(BAION,STEQION,LU_BA,NION*NT*NUM_BNDS*ND,NION*ND,
	1              NLBEGIN,COMPUTE_BA,'BAION')
	END IF
C
C Allow for adiabatic cooling, if requested.
C
	CALL EVAL_ADIABATIC(BA,STEQ,AD_COOL_V,AD_COOL_DT,
...
C
	    CALL WR_CHG_COOL(TA,TB,LS,ND,LU_REC_CHK)
C
...
C
	IF(LST_ITERATION)
	1     CALL WR2D(STEQ,NT,ND,'STEQ ARRAY- Continuum Terms',19)
C                                   
		CALL TUNE(ITHREE,' ')
C
		END IF		!NLBEGIN IF
...
C
	...
	1            )
	      BA(NUP,NUP,L,K)=BA(NUP,NUP,L,K) - dRATE_dUP
	      BA(NUP,NL,L,K)=BA(NUP,NL,L,K) - dRATE_dLOW
	      BA(NUP,NT,L,K)=BA(NUP,NT,L,K) - dRATE_dT
	      BA(NL,NUP,L,K)=BA(NL,NUP,L,K) + dRATE_dUP
	      BA(NL,NL,L,K)=BA(NL,NL,L,K) + dRATE_dLOW
	      BA(NL,NT,L,K)=BA(NL,NT,L,K) + dRATE_dT
	      BA(NT,NL,L,K)=BA(NT,NL,L,K) - T4*dRATE_dLOW
	      BA(NT,NUP,L,K)=BA(NT,NUP,L,K) - T4*dRATE_dUP
	      BA(NT,NT,L,K)=BA(NT,NT,L,K) - T4*dRATE_dT
	    END DO
C
	  END IF
C
	        ...
	        DO K=BNDST(L),BNDEND(L)	 	!Variable depth.
	          LS=BND_TO_FULL(K,L)
	            ...
	            IF( DABS(VJ(J,K,L)*POPS(J,LS)) .GE. T3 )THEN
	              T4=T2*VJ(J,K,L)
	              BA(NL,J,K,L)=BA(NL,J,K,L) - T4/T1
	              BA(NUP,J,K,L)=BA(NUP,J,K,L) + T4/T1
	              BA(NT,J,K,L)=BA(NT,J,K,L) + T4
	            END IF
	          END DO
	        END DO
	      END DO
...
C                                    
	...
	1               EMIS_FAC*VC_2(K)*BB_COR(K,J)+STIM_FAC*VB_2(K)) )
	        END IF
	        BA(NUP,SIM_NUP(J),L,K)=BA(NUP,SIM_NUP(J),L,K) - dRATE_dUP
	        BA(NUP,SIM_NL(J),L,K)=BA(NUP,SIM_NL(J),L,K) - dRATE_dLOW
	        BA(NUP,NT,L,K)=BA(NUP,NT,L,K) - dRATE_dT
	        BA(NL,SIM_NUP(J),L,K)=BA(NL,SIM_NUP(J),L,K) + dRATE_dUP
	        BA(NL,SIM_NL(J),L,K)=BA(NL,SIM_NL(J),L,K) + dRATE_dLOW
	        BA(NL,NT,L,K)=BA(NL,NT,L,K) + dRATE_dT
	        BA(NT,SIM_NL(J),L,K)=BA(NT,SIM_NL(J),L,K) - T4*dRATE_dLOW
	        BA(NT,SIM_NUP(J),L,K)=BA(NT,SIM_NUP(J),L,K) - T4*dRATE_dUP
	        BA(NT,NT,L,K)=BA(NT,NT,L,K) - T4*dRATE_dT
	      END DO
	    END DO
	  END DO
...
C
	...
	1          .AND. .NOT. FLUX_CAL_ONLY 
	1          .AND. .NOT. LAMBDA_ITERATION)THEN
	  CALL STOREBA(BA,STEQ,LU_BA,NT*NT*NUM_BNDS*ND,
	1          NT*ND,NT,COMPUTE_BA,'BAMAT')
	  CALL STOREBA(BAION,STEQION,LU_BA,NION*NT*NUM_BNDS*ND,
	1          NION*ND,NT,COMPUTE_BA,'BAION')
	END IF
C
...
C
	...
	1   'Total Dielectronic and Implicit Recombination Luminosity:',T2
	  ...
	  WRITE(LU_FLUX,'(A,T60,1PE12.4)')'Total Shock Luminosity:',SUM(XRAY_LUM_TOT)
	  WRITE(LU_FLUX,'(A,T60,1PE12.4)')'X-ray Luminosity (> 0.1 keV) :',SUM(XRAY_LUM_0P1)
	  WRITE(LU_FLUX,'(A,T60,1PE12.4)')'X-ray Luminosity (> 1 keV):',SUM(XRAY_LUM_1KEV)
	CLOSE(UNIT=LU_FLUX)
C
...
C 
C
C Replace ground state equations if nearly singular (with n=2 or n=3
C equation) by ionization balance equation. At present not all species
C are installed.
C
C NB: EDGE"_F is no longer accessed in BA_REPLACE, and hence is a dummy
C      variable.
C
	DO ID=1,NUM_IONS-1
	  CALL BA_REPLACE(BA,STEQ,BAION,STEQION,
	1          ATM(ID)%XzV, ATM(ID)%DXzV, ATM(ID)%EDGEXzV_F,T,
	1          ATM(ID)%NXzV, ATM(ID)%EQXzV, ATM(ID)%EQXzVBAL,
	1          NION,NT,NUM_BNDS,ND, 
	1          ION_ID(ID), ATM(ID)%XzV_PRES)
	END DO
C
C
C
C Fix those populations which we want to remain fixed.  We loop through
C for each depth to prevent excessive accessing of the BA array. If memory
C is not a problem, DST may be replaced by IONE, DEND by ND, and the DO
C LOOP can be removed. There are problems with these routines if
C high ionization stages are missing. May not fix ion. An alternative
C approach, probably cleaner, would be to set up a FIX_VEC which would
C indicate those levels to be held fixed (at each depth?)
C
C	DO L=1,ND
C	  DST=L
C	  DEND=L
...
C
	  DO ID=1,NUM_IONS-1
	    CALL FIXPOP(BA,STEQ,NT,NUM_BNDS,ND,DST,DEND,
	1          ATM(ID)%EQXzV, ATM(ID)%NXzV, ATM(ID)%FIX_NXzV,ION_ID(ID),
	1          ATM(ID)%XzV,POP_SPECIES(1,SPECIES_LNK(ID)), ATM(ID)%XzV_PRES,
	1          FIX_IMPURITY)
	  END DO
	  DO ISPEC=1,NUM_SPECIES
	    IF(SPECIES_PRES(ISPEC))THEN
	      TA(1:ND)=POPS(EQ_SPECIES(ISPEC),1:ND)
	      CALL FIXPOP(BA,STEQ,NT,NUM_BNDS,ND,DST,DEND,
	1             EQ_SPECIES,IONE,FIX_SPECIES(ISPEC),SPECIES,
	1             TA,POP_SPECIES(1,ISPEC),SPECIES_PRES(ISPEC),FIX_IMPURITY)
	    END IF
	  END DO
C
C TA and TB are irrelevant in this call.
C
	IF(FIXED_NE)THEN
	  J=NT-1
	  TA(:)=0.0D0
	  ...
	  CALL FIXPOP(BA,STEQ,NT,NUM_BNDS,ND,DST,DEND,
	1                     J,IONE,IONE,'Ne',
	1                     TA,TB,L_TRUE,L_FALSE)
	END IF
C
C Fixed temperature? If either FIXED_T of RD_FIX_T is true, the temperature is 
C fixed independent of VARFIXT. We zero all elements of the R.E. Eq. except
C that local variation with respect to T. For a LAMBDA iteration this element
C is zero, and hence must be set. If we set to a very large number (not unity)
C it might not to necessary to zero all elements when not doing a LAMBDA 
C iteration. TA is a dummy vector when fixing 'T'.
C
	  IF(FIXED_T .OR. RD_FIX_T)THEN
	    CALL FIXPOP(BA,STEQ,NT,NUM_BNDS,ND,DST,DEND,
	1                     NT,IONE,IONE,'T',
	1                     TA,TA,L_TRUE,L_FALSE)
	  ELSE IF(VARFIXT .AND. CON_SCL_T .NE. 0.0D0)THEN
	    CALL ESTAU(TA,R,ED,TB,ND)
	    DO I=DST,DEND
	      IF(TA(I) .LT. TAU_SCL_T)THEN
	        CALL FIXPOP(BA,STEQ,NT,NUM_BNDS,ND,I,I,
	1                     NT,IONE,IONE,'T',
	1                     TA,TA,L_TRUE,L_FALSE)
	      END IF
	    END DO
	    FMT='('' Temperature held partially fixed: TAU_SCL_T='','//
	1            'F10.3)'
	    IF(L .EQ. 1)WRITE(LUER,FMT)TAU_SCL_T
	  END IF
	END DO
C 
C
...
C
	...
	CONTINUE
	CALL WR2D_V2(STEQ,NT,ND,'STEQ ARRAY','*',L_TRUE,LU_SE)
C
	IF(LST_ITERATION)THEN
	  OPEN(UNIT=LU_BA,FILE='BA_ASCI_D1',STATUS='UNKNOWN')
	    CALL WR2D(BA(1,1,DIAG_INDX,1),NT,NT,'BA ARRAY (d=1)',LU_BA)
	    CALL WR2D(BA(1,1,DIAG_INDX,10),NT,NT,'BA ARRAY (d=10)',LU_BA)
	  CLOSE(UNIT=LU_BA)
...
C
	...
	ELSE
	  ...
	  TEMP_CHAR=METH_SOL
	END IF
	CALL SOLVEBA_V3(BA,STEQ,BA_PAR,POPS,
	1       DIAG_INDX,NT,NUM_BNDS,ND,REPA,MATELIM,
	1       MAXCH,TEMP_CHAR,SUCCESS,SCALE_OPT,T1,LAMBDA_ITERATION)
C
C Switch off BA computation of MAXCH if less than VAL_FIX_BA.
C We only do this provided the last iteration was not a LAMBDA
C iteration, and if T was not partially held fixed at some depths.
C
C We now only right out the BA matrix if we are nearing the correct 
C solution, and if it is NOT a LAMBDA iteration. The check was
C moved, as
C
C
C Switch off BA computation of MAXCH if less than VAL_FIX_BA.
...
C iteration.
C
C We now only right out the BA matrix if we are nearing the correct solution,
C and assuming that it is a full solution matrix (i.e. not from a 
C LAMBDA iteration and CON_SCL_T was not set).
C
	IF(N_ITS_TO_FIX_BA .GT. 0 .AND. .NOT. LAMBDA_ITERATION)THEN
	  IF(COMPUTE_BA)CNT_FIX_BA=0
...
C
	IF(.NOT. LAMBDA_ITERATION)THEN
	  ...
	  DO I=1,ND
	    DO J=1,NT
	      IF( (STEQ(J,I) .GT. 0.8D0 .OR.
	1          STEQ(J,I) .LT. -5.0D0) .AND. TA(I) .LT. 1.0)THEN
	            TAU_SCL_T=TA(I)
	            CON_SCL_T=1000.0D0
	      END IF
	      IF( (STEQ(J,I) .GT. 10.0D0 .OR.
	1          STEQ(J,I) .LT. -20.0D0) .AND. TA(I) .GE. 1.0)THEN
	            TAU_SCL_T=TA(I)
	            CON_SCL_T=1000.0D0
	      END IF
	    END DO
...
!	END IF
C
	CALL WR2D_V2(STEQ,NT,ND,'STEQ SOLUTION ARRAY','#',L_TRUE,LU_SE)
C
...
C
C NEXT_NG=1000 indicates a NEW model, or that MAXCH is again above VAL_DO_NG
...
C
	IF( MAXCH .LT. VAL_DO_NG .AND. NEXT_NG .EQ. 1000)THEN
...
C
C We now perform a LAMBDA iteration after each NG acceleration in case
C the NG acceleration has cause large population changes.
C	  
	IF(NG_DO .AND. (.NOT. LST_ITERATION) .AND.
	1       (NEXT_NG .EQ. MAIN_COUNTER) )THEN
	  CALL GENACCEL(POPS,T1,T2,I,NT,ND)
	  IF(I .EQ. 0)THEN
	    MAIN_COUNTER=MAIN_COUNTER+1
...
C
	I=-1000
	CALL INIT_BA_PNT(LU_BA,NT*NT*NUM_BNDS*ND,NT*ND,I,COMPUTE_BA,'BAMAT')
	CALL INIT_BA_PNT(LU_BA,NION*NT*NUM_BNDS*ND,
	1          NION*ND,I,COMPUTE_BA,'BAION')
C
C
C 
C
