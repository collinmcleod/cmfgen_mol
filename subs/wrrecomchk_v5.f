...
C
C
	SUBROUTINE WRRECOMCHK_V5(PR,RR,CPR,CRR,CHG_PR,CHG_RR,MOL_DR,MOL_CR,ADVEC_RR,
	...
	IMPLICIT NONE
C
C Altered 18-Apr-2022 : Collin McLeod added molecular reaction creation/destruction rates
C                     : Changed to v5
...
C
	...
	REAL(KIND=LDP) MOL_DR(ND)               !Destruction rate by mol rxns
	REAL(KIND=LDP) MOL_CR(ND)               !Creation rate by mol rxns
	...
	REAL(KIND=LDP) TOTRR(ND),NETRR(ND),NETRR2(ND)
	REAL(KIND=LDP) R(ND),T(ND),ED(ND),DHYD(ND)
	CHARACTER*(*) FILNAM,STRDESC
	REAL(KIND=LDP) ABS_SUM, ABS_SUM2
...
C
	  DO J=MS,MF
	    ...
	    DO I=1,N
	      ...
	      NETRR2(J)=NETRR2(J)+(RR(I,J)-PR(I,J))
	      TOTRR(J)=TOTRR(J)+RR(I,J)
	      ABS_SUM=ABS_SUM+RR(I,J)+PR(I,J)
	      ABS_SUM2=ABS_SUM2+RR(I,J)+PR(I,J)
	    END DO
	...
	1             ABS(NT_ION_RATE(J))
	    ABS_SUM2=ABS_SUM2+CRR(J)+CPR(J)+
	1             CHG_PR(J)+CHG_RR(J)+MOL_CR(J)+MOL_DR(J)+
	1             ABS(DIERECOM(J))+ABS(ADDRECOM(J))+ABS(ADVEC_RR(J))+
	1             ABS(X_RECOM_1(J))+ABS(X_RECOM_2(J))+
	1             ABS(NT_ION_RATE(J))
	...
	1                    NT_ION_RATE(J))/ABS_SUM
	    NETRR2(J)=200.0_LDP*(NETRR2(J)+(CRR(J)-CPR(J))+
	1                    (CHG_RR(J)-CHG_PR(J))+(MOL_CR(J)-MOL_DR(J))+
	1                    DIERECOM(J)+ADDRECOM(J)+ADVEC_RR(J)+
	1                    X_RECOM_1(J)+X_RECOM_2(J)-
	1                    NT_ION_RATE(J))/ABS_SUM2
	  ...
	  END DO
C
	  WRITE(LU,'(/3X,''Net Recombination Rate (% of total, w/o mol rxns) '') ')
	  WRITE(LU,999)(NETRR(J),J=MS,MF)
	  WRITE(LU,'(/3X,''Net Recombination Rate (% of total, w/ mol rxns) '') ')
	  WRITE(LU,999)(NETRR2(J),J=MS,MF)
