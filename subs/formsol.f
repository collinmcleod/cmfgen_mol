...
C
	...
	REAL(10) JEX_SCAT(ND),BAR_WTS(ND),DEPTH_PRO(ND)
	REAL(10) HCONT(ND),JINT_PREV(ND),HINT(ND)
	REAL(10) AVCONT(ND),CVCONT(ND),dCHIdR(ND)
	REAL(10) AVM1(ND),CVM1(ND),dJBARdKI(ND),DIAG(ND)
	REAL(10) VSRCE(ND),dUdS(ND),dVdS_UP(ND),dVdS_LOW(ND),dJINTdS(ND)
	REAL(10) VKI(ND),RKB(ND),dUdKI(ND),dVdKI_UP(ND),dVdKI_LOW(ND)
C
C Local variables.
C
	INTEGER I,LS,ML,NI,NIEXT
	REAL(10) OLDCHI,T1,T2,DBC,DNU,TOR,IBOUND,WERF_EXP,SCALE
C
