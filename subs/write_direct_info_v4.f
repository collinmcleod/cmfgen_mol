...
!
! Modified 06-Dec-2023: Added REAL_SIZE to output.
...
!
	SUBROUTINE WRITE_DIRECT_INFO_V4(ND,RECL,FILE_DATE,FILENAME,LU_EDD)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
! Created 16-Jun-2000
...
!
	...
	NEW_FILENAME=TRIM(FILENAME)//'_INFO'
        OPEN(UNIT=LU_EDD,FILE=NEW_FILENAME,STATUS='REPLACE')
          WRITE(LU_EDD,'(X,A,39X,A)')'06-Dec-2023','!INFO format date'
          WRITE(LU_EDD,'(X,A,39X,A)')TRIM(FILE_DATE),'!File format date'
          WRITE(LU_EDD,'(6(6X,I6),11X,1L)')ND,RECL,WORD_SIZE,UNIT_SIZE,INT_SIZE,REAL_SIZE,CHK_LITTLE_ENDIAN()
          WRITE(LU_EDD,'(6(3X,A))')'       ND','     RECL','WORD_SIZE','UNIT_SIZE',' INT_SIZE','REAL_SIZE','  LIT_END'
        CLOSE(LU_EDD)
!
	RETURN
	END
...
!
	SUBROUTINE READ_DIRECT_INFO_V4(ND,RECL,FILE_DATE,FILENAME,LU_EDD,IOS)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
...
!
	...
	EXTERNAL ERROR_LU
!
! Set standard default sizes.
!
	INIT_SIZE=4; REAL_SIZE=8
	CALL DIR_ACC_PARS(REC_SIZE_LIM,UNIT_SIZE,WORD_SIZE,MAX_NUM_REC)
!
	IOS=0
	J=LEN_TRIM(FILENAME)
...
!
	  ...
	  ELSE
	    IF(INDEX(STRING,'06-Dec-2023'))THEN
	      ...
	      IER=5; READ(LU_EDD,*,ERR=100,IOSTAT=IOS)ND,RECL,OLD_WORD_SIZE,OLD_UNIT_SIZE,INT_SIZE,REAL_SIZE
	      RECL=RECL*WORD_SIZE*OLD_UNIT_SIZE/OLD_WORD_SIZE/UNIT_SIZE
!
	    ELSE IF(INDEX(STRING,'18-Aug-2003') .NE. 0 .OR. INDEX(STRING,'12-Apr-2017') .NE. 0)THEN
	      IER=3; READ(LU_EDD,'(A)',ERR=100,IOSTAT=IOS)STRING
	      IER=4; IF(INDEX(STRING,'File format date') .EQ. 0)GOTO 100
	      FILE_DATE=STRING(1:INDEX(STRING,'  '))
	      IER=5; READ(LU_EDD,*,ERR=100,IOSTAT=IOS)ND,RECL,OLD_WORD_SIZE,OLD_UNIT_SIZE
	      RECL=RECL*WORD_SIZE*OLD_UNIT_SIZE/OLD_WORD_SIZE/UNIT_SIZE
	      IF(KIND(X) .NE. REAL_SIZE)THEN
	         WRITE(6,*)'Warning in READ_DIRECT_INFO -- add REAL_SIZE in XXX_INFO'
	         WRITE(6,*)'If inconsistent, code may crash'; FLUSH(UNIT=6)
	      END IF
!
...
!
100	J=ERROR_LU()
	WRITE(J,*)'Error in READ_DIRECT_INFO_V4'
