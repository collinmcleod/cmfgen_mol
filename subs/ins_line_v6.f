...
c
	...
	IMPLICIT NONE
C
C Created 03-Jan-2014: Essentially the same as INS_LINE_V5 except specing in resonance zone
C                         is actually FRAC_DOP rather than 05*FRAC_DOP. INS_LINE_V5 was left
C                         unchanged so that CMF_FLUX did not change.
C
...
C
	...
	IF(N_LINES .NE. LOCAL_N_LINES)THEN
	  WRITE(LU_ER,*)'Warning from INS_LINE_V6'
...
!	  NFREQ=J
!	  LU_ER=ERROR_LU()
!	  WRITE(LU_ER,*)'Warning --- no line inserted in INS_LINE_V6'
!
!         DO ML=2,NFREQ
...
C
	    ...
	    DO WHILE( (FREQ(INDX)-dNU_NEXT) .LE. NU_STRT_LINE(K) )
	      IF(TRANS_TYPE(K)(1:3) .EQ. 'BLA')THEN
	        dNU=FREQ(INDX)*FRAC_DOP*VEC_MIN_VDOP(K)*
...
C
	    ...
	    FREQ(INDX)=FREQ(INDX-1)-dNU_NEXT
C
C Set location of resonance zone. Because the reosnance zones can have
