...
!
	INTEGER*4, ALLOCATABLE :: J_PNT(:,:)
	INTEGER*4, ALLOCATABLE :: H_PNT(:,:)
!
...
!
	INTEGER*4, SAVE :: FREQ_CNT
	INTEGER*4 ACCESS_F
        INTEGER*4 IOS,REC_SIZE,UNIT_SIZE,WORD_SIZE,N_PER_REC
	LOGICAL WRITE_IP
!
...
!
	INTEGER*4 N_ERR_MAX,FG_ERR_CNT
	PARAMETER (N_ERR_MAX=1000)
	REAL*8 FG_ERR_ON_FREQ
	INTEGER*4 FG_ERR_TYPE
...
!
	...
	1       ACCESS='DIRECT',STATUS='UNKNOWN',RECL=I,IOSTAT=IOS)
	  FREQ_CNT=0
          ...
          WRITE(56,REC=ACCESS_F)(P(I),I=1,NP)
	  WRITE_IP=.TRUE.
	END IF
        IF(WRITE_IP)THEN
	  T1=0.0D0
