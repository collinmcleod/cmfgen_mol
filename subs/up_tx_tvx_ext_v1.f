...
C
	...
	REAL(10) TX(NDEXT,ND,NM_TX)
	REAL(10) TVX(NDEXT-1,ND,NM_TX)
	REAL(10) KI(NDEXT,ND,NM_KI)
	REAL(10) TA(NDEXT),TB(NDEXT),TC(NDEXT)
	REAL(10) PSIPREV_MOD(NDEXT),VB(NDEXT),VC(NDEXT)
	REAL(10) HU(NDEXT),HL(NDEXT),HS(NDEXT)
	REAL(10) RHS_dHdCHI(NDEXT-1,ND)
C
C NB: _A denotes that EPS(I) multiples RJ(I)
C     _B denotes that EPS(I) multiples RJ(I+1)
C
	REAL(10) EPS_A(NDEXT),EPS_B(NDEXT)
	REAL(10) EPS_PREV_A(NDEXT),EPS_PREV_B(NDEXT)
	LOGICAL INIT,DO_THIS_TX_MATRIX(NM_TX)
C
C Work Array.
C
	REAL(10) OLD_TX(NDEXT,ND)
C
	INTEGER ERROR_LU
	EXTERNAL ERROR_LU
C
