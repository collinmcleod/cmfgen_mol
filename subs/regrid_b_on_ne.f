...
!
	SUBROUTINE REGRID_B_ON_NE(DHEN,ED,ION,ELOG,N,NST,NFIN,ND,
	1                         LU,FILNAME)
	IMPLICIT NONE
!
...
!
	...
	REAL(10) DHEN(2-NST:N-NST+1,ND),ED(ND),ION(ND),ELOG(ND)
	CHARACTER*(*) FILNAME
!
...
!
	NXST=1
	DO WHILE (ELOG(NXST) .LT. OLDED(1))
	  NXST=NXST+1
	END DO
	NX_END=ND
	DO WHILE (ELOG(NX_END) .GT. OLDED(NDOLD))
	  NX_END=NX_END-1
	END DO
	NX=NX_END-NXST+1
!
...
!
	DO J=1,NZ
	  ...
	  END DO
	  CALL LINPOP(ELOG(NXST),TA(NXST),NX,OLDED,TB,NDOLD)
	  DO I=NXST,NX_END
	    DHEN(J,I)=EXP(TA(I))
	  END DO
	  IF(NXST .NE. 1)THEN
	    DO I=1,NXST-1
	      DHEN(J,I)=EXP(TB(3)+(ELOG(I)-OLDED(3))
...
! Regrid ion density.
!
	CALL LINPOP(ELOG(NXST),ION(NXST),NX,OLDED,OLDION,NDOLD)
	IF(NXST .NE. 1)THEN
	  DO I=1,NXST-1
	    ION(I)=OLDION(3)+(ELOG(I)-OLDED(3))
