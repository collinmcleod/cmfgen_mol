...
C
	...
	REAL(10) ETA(ND),CHI(ND),THETA(ND),ESEC(ND)
	REAL(10) CHIL(ND),ETAL(ND),JCONT(ND)
	REAL(10) V(ND),SIGMA(ND),R(ND)
C
C Radiation field variables.
C
	REAL(10) JNU(ND,NLF+1),RSQHNU(ND,NLF+1)
	REAL(10) F(ND,NLF+1),G(ND,NLF+1)
	REAL(10) HBC(3,NLF+1),NBC(3,NLF+1),IN_HBC(NLF+1)
	REAL(10) JBAR(ND),ZNET(ND),HBLANK(ND),JBLANK(ND)
C
C Profile information
C
	REAL(10) PF(NLF),PROF(NLF),LFQW(NLF)
C
	REAL(10) DBB,IC,FL,EW,CONT_INT,SRCEBND
	CHARACTER*6 METHOD
	LOGICAL DIF,LINE_BL,FULL_ES
C
	REAL(10) TA(ND),TB(ND),TC(ND),DTAU(ND),TCHI(ND),DTAUONQ(ND)
	REAL(10) Q(ND),XM(ND),SOURCE(ND),JEX_SCAT(ND)
	REAL(10) VB(ND),VC(ND),HU(ND),HL(ND),HS(ND)
	REAL(10) GAM(ND),GAMH(ND),W(ND),WPREV(ND)
