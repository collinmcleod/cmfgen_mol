...
C
	INTEGER NC,ND,NP
	REAL(KIND=LDP) TA(ND),TB(ND),TC(ND),XM(ND),DTAU(ND),dCHIdr(ND)
	REAL(KIND=LDP) R(ND),Z(ND),P(NP),SOURCE(ND),CHI(ND),SOB(ND)
	REAL(KIND=LDP) HAQW(ND,NP),S1,DBB,IC
	LOGICAL THICK,DIF
	CHARACTER*6 METHOD
C
