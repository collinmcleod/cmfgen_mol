...
C
	...
	REAL(10) TX(ND,ND,NM),TVX(ND-1,ND,NM),KI(ND,3,NM)
	REAL(10) JQW(ND,NP)
...
C
	...
	REAL(10) RKB(ND),RKC(ND),BAR_WTS(ND),DEPTH_PRO(ND)
	REAL(10) AVM1(ND),CVM1(ND),TXD(ND),TVXD(ND)
C
C Local variables.
C
...
C
	LOGICAL MLNE1
	INTEGER I,LS,ML,NI
	REAL(10) OLDCHI,T1,T2,DBC,DBC_ON_DBB,TOR,IBOUND,WERF_EXP
C
	FQAF(:,:,:)=0.0D0               !NM,ND,ND
	FQAFD(:)=0.0D0         		!ND
C
