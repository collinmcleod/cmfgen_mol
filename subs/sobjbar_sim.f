...
C
	...
	REAL(10) SOURCE(ND),CHI(ND),CHIL(ND),ETAL(ND)
	REAL(10) V(ND),SIGMA(ND),R(ND),P(NP),AQW(ND,NP)
C
	REAL(10) JBAR(ND)
	REAL(10) BETA(ND)
C
	REAL(10) ZNET(ND,NSIM)
	REAL(10) VB(ND,NSIM)
	REAL(10) VC(ND,NSIM)
	REAL(10) VB_2(ND)
	REAL(10) VC_2(ND)
	REAL(10) BETAC(ND,NSIM)
C
	REAL(10) CHIL_MAT(ND,NSIM)
	REAL(10) ETAL_MAT(ND,NSIM)
	REAL(10) BBCOR(ND,NSIM)
C
	REAL(10) DBB,IC,FL
	CHARACTER*(*) METHOD
	LOGICAL DIF,THICK
C
	...
	REAL(10) GAM(ND),GAMH(ND),dCHIdR(ND)
	REAL(10) EX_VEC(ND),dEX_VEC(ND)
	REAL(10) ZNET_BL(ND),BETAC_BL(ND)
	REAL(10) VB_BL(ND),VC_BL(ND)
	REAL(10) ST_ON_SL
C
	REAL(10) EXPONX,d_EXPONX_dX
C
C Local variables.
C
	INTEGER, PARAMETER :: IONE=1
	INTEGER I,J,LS,NI
	REAL(10) T1,T2,DBC,TOR
	REAL(10) IBOUND,E1,E2,E3  		!To handle NI=2
C
