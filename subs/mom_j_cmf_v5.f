...
C Altered:  05-Dec-1996  PROGDESC set to number not character. Keep as REAL(10)
...
C
	...
	REAL(10) JNU(ND),RSQHNU(ND)
	REAL(10) JNU_PREV(ND),RSQHNU_PREV(ND)
C
...
C
	...
	CHARACTER*6 METHOD
C
	REAL(10), PARAMETER :: PROG_ID=1.46310043D+08  !Must be unique (MOM_J_CM)
C
...
C
	REAL(10) CON_GAM(NV),CON_GAMH(NV),AV_SIGMA(NV)
...
C
	...
	REAL(10) TA(NV),TB(NV),TC(NV),DTAU(NV),DTAUONQ(NV)
	REAL(10) Q(NV),XM(NV),SOURCE(NV)
	REAL(10) VB(NV),VC(NV),HU(NV),HL(NV),HS(NV),COH_VEC(NV)
	REAL(10) GAM(NV),GAMH(NV),W(NV),WPREV(NV)
	REAL(10) PSI(NV),PSIPREV(NV)
	REAL(10) EPS(NV),EPS_PREV(NV)
	REAL(10) RSQJNU_PREV(NV)
C
	REAL(10) PROGDESC	
C
