...
C
	IF(MOD_DO_LEV_DIS)THEN
	  DO LEV=1,NC2
	    T1=3.289395*ZC2*ZC2/EDGE_C2(LEV)
	    NEFF=0.0D0
	    IF(T1 .GT. 0)NEFF=SQRT(T1)
	    IF(NEFF .LE. 2.01*ZC2)THEN			!0.01 allows for atomic mass
	      DO I=1,ND
	        W_C2(LEV,I)=1.0D0
	      END DO
	    ELSE
	      REAL_K=16.0D0*NEFF/(1+NEFF)/(1+NEFF)/3 .0D0
	      IF( NEFF .LE. 3)REAL_K=1.0D0
	      REAL_K=( REAL_K/ZC2*(ZC2/NEFF)**4 )**1.5D0
	      DO I=1,ND
	        Y=1.091*(X_LEV_DIS(I)+4.0D0*(ZC2-1)*A_LEV_DIS(I))
	        BETA=REAL_K*B_LEV_DIS(I)
	        F=Y*BETA*BETA/(7.782+X_LEV_DIS(I)*BETA)
