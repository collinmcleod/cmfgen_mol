...
C
	SUBROUTINE RITE_ATMDES_V3(CI_PRES,
	1                      NCI_S,ZCI,EQCI,
...
C
	...
	1                N_CI_PHOT,EQCI,TRIM(CI_LEVNAME_F(NCI_F))
	ELSE
	  STAR='****'
...
C
100	FORMAT(1X,A,T12,A4,:2(2X,I4):,4(3X,F5.1,1X),4X,I2,4X,I4,4X,A)
C
	RETURN
	END
 
	SUBROUTINE RITE_ATMHD_V3(LUMOD)
	IMPLICIT NONE
	INTEGER LUMOD
C
	WRITE(LUMOD,100)
