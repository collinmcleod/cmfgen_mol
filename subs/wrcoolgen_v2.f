...
C
	SUBROUTINE WRCOOLGEN_V2(BFCRC2,FFC2,COLC2,DIECOOL,X_COOL,ARTIFICIAL_HEAT_TERM,
	1                      C2_PRES,NC2,DESC,NETRR,TOTRR,
	1                      COUNTER,ND,LU)
	IMPLICIT NONE
C
C Altered 27-Sep-2004 - Added ARTIFICAL_HEAT_TERM to call, and changed to V2.
...
C
	CHARACTER*(*)DESC
	LOGICAL C2_PRES
	INTEGER*4 NC2,ND,COUNTER,LU,IOS
...
C
	INTEGER*4 ICHRLEN,ERROR_LU,LUER
	EXTERNAL ICHRLEN,ERROR_LU
C
C Local variables.
C
	INTEGER*4 MS,MF,I,J,L
...
C
	...
	END IF
!
! If we are forcing T > T_MIN, there is an artifical heating term which we have
! added.
!
	IF(ARTIFICIAL_HEAT_TERM(1) .NE. 0 .OR. ARTIFICIAL_HEAT_TERM(ND) .NE. 0)THEN
	  WRITE( LU,'(/,X,A)')'Artifical heating term'
	  WRITE(LU,999)(ARTIFICIAL_HEAT_TERM(J),J=MS,MF)
	END IF
!
	DO J=MS,MF
	  DO I=1,NC2
	  ...
	  END DO
	  NETRR(J)=NETRR(J)+FFC2(J)+COLC2(J)+DIECOOL(J)+X_COOL(J)+ARTIFICIAL_HEAT_TERM(J)
	  TOTRR(J)=TOTRR(J)+ABS(FFC2(J))+ABS(COLC2(J))+
	1            ABS(DIECOOL(J))+ABS(X_COOL(J))+ABS(ARTIFICIAL_HEAT_TERM(J))
	END DO
C
	RETURN
