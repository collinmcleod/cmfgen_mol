...
C
C
	INTEGER*4 NC2,EQC2,INDX_C2
	INTEGER*4 NMAX,NUM_DIE,LUIN,LUOUT
C
	REAL*8 GION			!St. Weight of ion (eg CIII).
	REAL*8 EDGEDIE(NMAX)
	REAL*8 EINADIE(NMAX)
	REAL*8 GUPDIE(NMAX)
	INTEGER*4 LEVDIE(NMAX)  	!Indicates NL of low state
	INTEGER*4 INDXDIE(NMAX)		!Species identification.
C
	LOGICAL DO_WI
	LOGICAL DO_AUTO
C
...
C Local variables
C
	INTEGER*4 I,LOOP,IOS,L1,L2,LUER
	INTEGER*4 MNL,ML,NUM_D_RD
