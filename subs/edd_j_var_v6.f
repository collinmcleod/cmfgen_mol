...
!
	INTEGER ND,NM
	REAL(10) VK(ND,ND,NM),RHS_dHdCHI(ND-1,ND)
	REAL(10) dTAUdCHI(ND,ND)
	REAL(10) SOURCE(ND),CHI(ND),ESEC(ND),ES_COH_VEC(ND)
	REAL(10) DTAU(ND),R(ND),SIGMA(ND)
	REAL(10) MIDF(ND),Q(ND),HU(ND),HL(ND),HS(ND),RSQ_DTAUONQ(ND)
	REAL(10) W(ND),WPREV(ND),PSI(ND),PSIPREV(ND)
	REAL(10) EPS_A(ND),EPS_B(ND)
	REAL(10) EPS_PREV_A(ND),EPS_PREV_B(ND)
	REAL(10) JNU(ND),JNUM1(ND),RSQ_HNUM1(ND)
	REAL(10) DBB
	REAL(10) HBC
	LOGICAL DIF
	INTEGER OUT_BC_TYPE
...
! Local variables.
!
	REAL(10) dHUdCHI(ND),dHLdCHI(ND),dHSdCHI(ND)
	REAL(10) dHUdTAU(ND),dHLdTAU(ND),EPS_FAC(ND)
...
!
	...
	REAL(10) dXM_EPS_J,dXM_EPS_I,dXM_EPS_K
!
! 
!
	IF(NM .LT. 2)THEN
