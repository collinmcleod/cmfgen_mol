...
C
	...
	REAL(10) CHIL(ND),ETAL(ND)
	REAL(10) V(ND),SIGMA(ND),R(ND)
C
C Varaiation arrays and vectors.
C
	REAL(10) FQAF(ND,ND,NM),FQAFD(ND)
	REAL(10) TX(ND,ND,NM),TVX(ND-1,ND,NM)
	REAL(10) KI(ND,ND,NM),WORKMAT(ND,ND),RHS_dHdCHI(ND-1,ND)
C
...
C
	REAL(10) TA(NV),TB(NV),TC(NV),DTAU(NV),TCHI(NV),DTAUONQ(NV)
	REAL(10) XM(NV),SOURCE(NV),MIDF(NV),Q(NV)
	REAL(10) JNU(NV),HNU(NV),JNUM1(NV),HNUM1(NV),TXD(NV),TVXD(NV)
	REAL(10) BAR_WTS(NV),DEPTH_PRO(NV)
	REAL(10) VB(NV),VC(NV),HU(NV),HL(NV),HS(NV)
	REAL(10) GAM(NV),GAMH(NV),W(NV),WPREV(NV)
	REAL(10) PSI(NV),PSIPREV(NV)
C
	REAL(10) PROGDESC	
	REAL(10), PARAMETER :: PROG_ID=2.22814021D+08 !Must be unique (VAR_MJBA)
C
