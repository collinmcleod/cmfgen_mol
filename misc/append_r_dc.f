...
C
	...
	INTEGER*4, PARAMETER :: LUIN=10		!File input
	INTEGER*4, PARAMETER :: LUSCR=11	!
	INTEGER*4, PARAMETER :: DCIN_O=15
	INTEGER*4, PARAMETER :: DCIN_N=16
	INTEGER*4, PARAMETER :: DCOUT=17
...
C
	...
	DO K=1,N_ADD
	  ...
	  WRITE(DCOUT,'(X,1P,5E15.5)')(DC_O(I,K),I=1,N_NEW)
	END DO
!
	DO K=1,ND_N
	  WRITE(DCOUT,'(A)')' '
	  WRITE(DCOUT,'(A)')TRIM(HEAD(K))
	  WRITE(DCOUT,'(X,1P,5E15.5)')(DC_N(I,K),I=1,N_NEW)
	END DO
C
	STOP
	END
