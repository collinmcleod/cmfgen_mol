...
!
	REAL(KIND=LDP), ALLOCATABLE :: FOSC(:,:)
!
...
! not used otherwise.
!
	REAL(KIND=LDP) EDGEXzSIX(1)
	REAL(KIND=LDP) GXzSIX(1)
	REAL(KIND=LDP) F_TO_S_XzSIX(1)
	INTEGER NXzSIX
	CHARACTER(LEN=40) XzSIX_LEV_NAME(1)
!
! Variables/vectors for computing recombination rates.
!
	REAL(KIND=LDP) TEMP(N_TEMP)
	REAL(KIND=LDP) RECOM(N_MAX,N_TEMP)
!
	LOGICAL XRAYS
	LOGICAL PACK
	LOGICAL DO_PHOT_SEP
