...
!
	IF(XOPT .EQ. 'RDM' .OR. XOPT .EQ. 'RD_MOD')THEN
	  ...
	  IF(ATM(NMOD)%DIR_NAME .NE. DIR_DELIMITER())ATM(NMOD)%DIR_NAME(K+1:K+1)=DIR_DELIMITER()
...
!
	  ...
	  READ(10,'(A)')STRING
	  IF(INDEX(STRING,'dTPdR/ROH') .NE. 0)OLD_FORMAT=.FALSE.
	  ATM(ID)%SOUND=0.0D0
	  DO I=1,ATM(ID)%ND
	    IF(OLD_FORMAT)THEN
	      ATM(ID)%dTPdR(I)=0.0D0
	      READ(10,*)ATM(ID)%R(I),ATM(ID)%VEL(I),T1,ATM(ID)%VdVdR(I),
	1          ATM(ID)%dPdR(I),ATM(ID)%GTOT(I),ATM(ID)%GRAD(I),
	1          ATM(ID)%GELEC(I),ATM(ID)%GAM(I),T1,T1,ATM(ID)%SOUND(I)
	    ELSE
	      READ(10,*)ATM(ID)%R(I),ATM(ID)%VEL(I),T1,ATM(ID)%VdVdR(I),
	1          ATM(ID)%dPdR(I),ATM(ID)%dTPdR(I),ATM(ID)%GTOT(I),ATM(ID)%GRAD(I),
	1          ATM(ID)%GELEC(I),ATM(ID)%GAM(I)
	    END IF
