...
!
	...
	IF(NPLT .EQ. 0)THEN
	  ...
	  READ(20,*)NPLT
	  DO L=1,NPLT
	    READ(20,'(A)')TMP_STR; TMP_STR=ADJUSTL(TMP_STR)
	    WRITE(6,'(A)')TRIM(TMP_STR)
	    DO K=1,NREC
	  ...
	  CLOSE(UNIT=20)
	ELSE
	  OPEN(UNIT=20,FILE='POINTER',STATUS='UNKNOWN',ACTION='WRITE')
	  WRITE(20,*)NPLT
	  DO L=1,NPLT
	    INDX=MAXLOC(ABS(VRATE))
	    POINT(L)=INDX(1)
	    WRITE(6,'(A10,2X,ES12.4)')TRIM(LABEL(POINT(L))),VRATE(INDX(1),INDX(2))
	    VRATE(INDX(1),:)=0.0D0
...
!
	...
	T1=0.0D0; T2=0.0D0
	DO L=1,NPLT
	  IF(L .NE. I_DECAY .AND. POINT(L) .NE. 0)THEN
	...
	END DO
	CALL SET_PG_TITLES(TITLE,L,L_TRUE)
	WRITE(6,'(A)')' '
