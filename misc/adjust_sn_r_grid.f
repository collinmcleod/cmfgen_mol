...
!
	SUBROUTINE ADJUST_SN_R_GRID(R,OLD_R,OLD_TAU,R_SCALE_FAC,
	1           IB_RAT,OB_RAT,N_IB_INS,N_OB_INS,ND,NS)
	IMPLICIT NONE
!
! Altered: 16-Jul-2013
!
...
!
	LOG_TAU_MIN=LOG_OLD_TAU(1)+(N_OB_INS-1)*LOG(OB_RAT_LOC)
	I=1
...
!
	  ...
	  IF(T2 .GT. dTAU)THEN
	    J=1
	    DO WHILE(TAU_END .LT. LOG_OLD_TAU(J+1))
	      J=J+1
	    END DO
...
!
	...
	END DO
	WRITE(6,*)'Defined the first grid',I
!
! Place points at the inner boundary.
!
	ND_TMP=I+1
...
!
	...
	END DO
	I=ND_TMP
	WRITE(LU,'(I6,ES18.8,6ES14.4)')I,EXP(LOG_R(I)),LOG_R(I),0.0D0,TAU(I),LOG_TAU(I),0.0D0
!
...
!
	  ...
	  END DO
	  T1=LOG_R(1)-LOG_R(3)
	  T2=LOG_TAU(1)-LOG_TAU(3)
	  LOG_R(1)=LOG_OLD_R(1)
	  LOG_R(2)=LOG_OLD_R(1)-0.02D0*T1
	  LOG_TAU(1)=LOG_OLD_TAU(1)
	  LOG_TAU(2)=LOG_OLD_TAU(1)-0.02D0*T2
	ELSE
!
! Shift grid to allow for insertion of extra ponts
!
	...
	END DO
	  ...
	  END DO
	  LOG_R(1)=LOG_OLD_R(1)
	  LOG_R(2)=LOG_R(1)-0.02D0*T1
	  LOG_TAU(1)=LOG_OLD_TAU(1)
	  LOG_TAU(2)=LOG_TAU(1)-0.02D0*T2
	END IF
