...
!
	...
	REAL(KIND=LDP), ALLOCATABLE :: MOL_RATE(:)
!
	INTEGER ND
	INTEGER NV
	INTEGER I,J,K,ID
...
!
	...
	ALLOCATE (MOL_RATE(ND));    MOL_RATE(:)=0.0_LDP
!
!
	     ...
	     ELSE IF( INDEX(STRING,'Net') .NE. 0)THEN
	      ...
	      ELSE IF( INDEX(STRING,'Charge exchange cooling rate') .NE. 0)THEN
	        ...
	        DO WHILE(1 .EQ. 1)
	          ...
	          END IF
	        END DO
	     ELSE IF ( INDEX(STRING,'Carbon Monoxide') .NE. 0) THEN
		TMP_STR=STRING
		SUM=0.0D0
		READ(20,'(A)') STRING
		CALL SUM_RATES(TOTAL_RATE,MOL_RATE,STRING,ID,ND)
		WRITE(21,'(A,T12,A)') 'COMOL',STRING
	      END IF
	   END DO
	END DO
	CLOSE(UNIT=20)
...
!
	  ...
	  WRITE(22,'(A,T12,10ES12.4)')' COMOL',(MOL_RATE(K),K=ID,MIN(ID+9,ND))
	  WRITE(22,'(A)')' '
