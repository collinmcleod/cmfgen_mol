!
! Data module for SET_PROF. It is used so that we don't have to compute
! the entire STARK profile (necessary for convolution purposes) on
! each call.
!
	MODULE PROF_MOD
	IMPLICIT NONE
!
! Created 21-Sep-1999
!
	INTEGER NSTORE_MAX
	INTEGER NFREQ_MAX
	INTEGER LUER
	REAL(10) C_KMS
	REAL(10) PI
!
	REAL(10), ALLOCATABLE :: PROF_STORE(:,:,:)
	REAL(10), ALLOCATABLE :: NU_STORE(:,:)
	REAL(10), ALLOCATABLE :: AMASS_STORE(:)
	REAL(10), ALLOCATABLE :: NU_ZERO_STORE(:)
!         
	INTEGER, ALLOCATABLE :: NL_STORE(:)
	INTEGER, ALLOCATABLE :: NUP_STORE(:)
	INTEGER, ALLOCATABLE :: LST_FREQ_LOC(:)
	INTEGER, ALLOCATABLE :: NF_STORE(:)
!
	REAL(10), ALLOCATABLE :: PR_GRIEM(:)
	REAL(10), ALLOCATABLE :: DWS_GRIEM(:)
!
	LOGICAL, ALLOCATABLE :: STORE_AVAIL(:)
!
	INTEGER NVGT_ST
	TYPE PROF_STORE_VGT
	  REAL(10), ALLOCATABLE :: VGT_PROF(:,:)
	  INTEGER NF
	END TYPE PROF_STORE_VGT
	INTEGER, ALLOCATABLE :: VGT_POINTER(:)
!
	TYPE (PROF_STORE_VGT), ALLOCATABLE :: VGT(:)
	
!
	END MODULE PROF_MOD
